...
if( isset($data['data_type'])) {
    ...
    $company = new Specilalization();
    $status = 200;

    if($data_type == "create") {

        $res = $company->addData($json);
        Library::setResponse(['status'=>201,'message'=>"Specialization added successfully"],201,'OK');
        return;
    }
        ...
        Library::setResponse(['status'=>201,'message'=>"Specialization updated successfully"],201,'OK');
        return;
    }

    else if($data_type == "delete") {

        $student_id = isset($data['specialization_id']) ? $data['specialization_id'] : false;
        ...
        Library::setResponse(['status'=>201,'message'=>"Specilalization data removed successfully"],201,'OK');
        return;
