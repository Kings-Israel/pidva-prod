...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE pel_edu_courses SET faculty_id=%s WHERE course_id=%s",
                       GetSQLValueString($_POST['faculty_id'], "text"),
                       GetSQLValueString($_POST['faculty_id'], "int"));

  mysqli_select_db($connect,$database_connect);
  $Result1 = mysqli_query_ported($updateSQL, $connect) or die(mysqli_error($connect));

  $updateGoTo = "updateids.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$updateSQL = sprintf("UPDATE pel_edu_inst_courses SET faculty_id=%s WHERE faculty_name=%s",
                       GetSQLValueString($row_getfaulty['faculty_id'], "text"),
                       GetSQLValueString($row_getfaulty['faculty_name'], "text"));
