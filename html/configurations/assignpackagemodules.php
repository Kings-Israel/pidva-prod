...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "assignpackagemodule")) {
  mysqli_select_db($connect,$database_connect);
  
  if(!empty($_POST['module_id'])){
// Loop to store and display values of individual checked checkbox.
foreach($_POST['module_id'] as $selected){
$togetmoduledetails=$selected;
/*$togetmoduledetails = $_POST['module_id'];*/
$togetpackage = $_POST['package_id'];

mysqli_select_db($connect,$database_connect);
$query_getmoduledetails = "SELECT * FROM pel_module WHERE module_id='$togetmoduledetails'";
$getmoduledetails = mysqli_query_ported($query_getmoduledetails, $connect) or die(mysqli_error($connect));
$row_getmoduledetails = mysqli_fetch_assoc($getmoduledetails);
$totalRows_getmoduledetails = mysqli_num_rows($getmoduledetails);

$query_getpackagedetails = "SELECT * FROM pel_package WHERE package_id='$togetpackage'";
...
$insertSQL = sprintf("INSERT INTO pel_packages_module (module_name, module_id, status, added_by, added_date, package_id, package_name) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($row_getmoduledetails['module_name'], "text"),
                       GetSQLValueString($row_getmoduledetails['module_id'], "text"),
 ...
 $Result1 = mysqli_query_ported($insertSQL, $connect)or die(mysqli_error($connect));
  ...
  $insertGoTo = "viewpackagemodules.php?package_id=$package_id";
...
$errorcode ='<div class="alert alert-danger">
											...
											Please Choose Atleast One Module.
											<br />
										</div>';
...
mysqli_select_db($connect,$database_connect);
$query_getallpackages = "SELECT * FROM pel_package where package_status='11' ORDER BY package_name ASC";
...
mysqli_select_db($connect,$database_connect);
$query_getmodules = "SELECT * FROM pel_module where status='11' ORDER BY module_name ASC";
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Assign  Module to Package - Peleza Admin</title>
...
<div class="row">
                                    ...
                                    <div  class="col-xs-6">
									  ...
									  <h3 align="left" class="header smaller lighter blue">ASSIGN MODULES TO PACKAGES</h3>
                                      </div>
								...
								<a href="psmtpackages.php">	
                                  <button class="btn btn-white btn-info btn-bold">
										  <i class="ace-icon bigger-120 green"></i>All Packages</button></a>&nbsp;</h3>
</div>
<div class="col-xs-12">
...
<div>
                                          	  ...
                                          	  <form method="POST" action="" class="form-horizontal" name="assignpackagemodule" >
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="added_date" name="added_date" value="<?php echo date('d-m-Y H:m:s');?>"/>
															...
															<div class="form-group">
																<label class="col-sm-3 control-label no-padding-right" for="form-field-pass2">Select Package</label>

															  <div class="col-sm-6"><span id="spryselect1">
															   	<select class="chosen-select form-control"  name="package_id" id="package_id" data-placeholder="Choose Package...">
															
														        <option value="000"></option>
														 <!--     <select name="package_id" id="package_id">
														        <option value="000">Select Institution</option>-->
														         <?php do { ?> 
                                                                 <option value="<?php echo $row_getallpackages['package_id']; ?>"><?php echo $row_getallpackages['package_name']; ?></option> 
											...
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
                                                  ...
                                                  <tr>
                                                     	<td class="center">
														<label class="pos-rel">
																<input type="checkbox" class="ace" name="module_id[]" id="module_id[]" value="<?php echo $row_getmodules['module_id']; ?>">
																<span class="lbl"></span>												</label>														</td>
                                                      
                                               <td>
                                                        <a href="#"><?php echo $row_getmodules['module_name']; ?></a>														</td>
                                           <td><?php echo $row_getmodules['module_code']; ?></td>
                                                     
                                                  
                                                      <td class="hidden-480"><?php 
														
														if($row_getmodules['status']=='11')
														{
														?>
                                                    ...
                                                    <?php } while ($row_getmodules = mysqli_fetch_assoc($getmodules)); ?>
...
<button class="btn" type="reset">
										        ...
										        <input type="hidden" name="MM_insert" value="assignpackagemodule">
							  </form>  
