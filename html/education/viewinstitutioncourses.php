...
$colname_getinstcourses = "-1";
if (isset($_GET['inst_id'])) {
  $colname_getinstcourses = $_GET['inst_id'];
}
mysqli_select_db($connect,$database_connect);
$query_getinstcourses = sprintf("SELECT * FROM pel_edu_inst_courses WHERE institution_id = %s ORDER BY faculty_name ASC", GetSQLValueString($colname_getinstcourses, "text"));
$getinstcourses = mysqli_query_ported($query_getinstcourses, $connect) or die(mysqli_error($connect));
$row_getinstcourses = mysqli_fetch_assoc($getinstcourses);
$totalRows_getinstcourses = mysqli_num_rows($getinstcourses);
...
if($_GET['status']=='11')
{
 $deleteSQL = sprintf("UPDATE pel_edu_inst_courses SET status=%s, verified_by=%s, verified_date=%s WHERE inst_course_id=%s",
  ...
  $deleteSQL = sprintf("UPDATE pel_edu_inst_courses SET status=%s WHERE inst_course_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
					   GetSQLValueString($_GET['inst_course_id'], "int"));
  ...
  $deleteGoTo = "viewinstitutioncourses.php?inst_id=$colname_getinstcourses";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>View Institution - Peleza Admin</title>
					  ...
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
						  ...
						  <a href="#">Education</a>							</li>
							 <li>
						  <a href="#">Institution</a></li><li class="active">View Institution</li>
</ul><!-- /.breadcrumb -->
  ...
  <tr>
    ...
    <td colspan="4"><br/></td>
							...
							<div class="clearfix">
									  ...
									  Results for "Courses Assigned configured"										</div>
											...
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
                                                  ...
                                                  <tr>
                                                      ...
                                                      <td>
                                                        <a href="#"><?php echo $row_getinstcourses['faculty_name']; ?></a>														</td>
                                                      <td><?php echo $row_getinstcourses['course_name']; ?></td>
                                                      <td><?php echo $row_getinstcourses['course_code']; ?></td>
                                                      <td><?php echo $row_getinstcourses['added_by']; ?></td>
                                                      <td><?php echo $row_getinstcourses['verified_by']; ?></td>
                                                   <td class="hidden-480"><?php 
														
														if($row_getinstcourses['status']=='11')
														{
														?>
														...
														if($row_getinstcourses['status']=='00')
														{
														?>
                                                        <span class="label label-sm label-danger">Deactivated</span>	
                                                         <?php
														}	
														if($row_getinstcourses['status']=='22')
														{
														?>
...
if (in_array('DEACTIVATE_MODULES_CONFIGURATION', $roledata)) 
{
														...
														{
														?>
                                                        <a href="viewinstitutioncourses.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&inst_id=<?php echo $row_getinstcourses['institution_id']; ?>&inst_course_id=<?php echo $row_getinstcourses['inst_course_id']; ?>"> <button class="btn btn-xs btn-danger">
																<i class="ace-icon fa fa-trash-o bigger-120"></i>															</button></a><?php
														}
														}
														
															 if (in_array('ACTIVATE_MODULES_CONFIGURATION', $roledata) && $row_getinstcourses['added_by']!=$_SESSION['MM_full_names']) 
{
														if($row_getinstcourses['status']=='00')
														{
														?>
                                                    <a href="viewinstitutioncourses.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&inst_id=<?php echo $row_getinstcourses['institution_id']; ?>&inst_course_id=<?php echo $row_getinstcourses['inst_course_id']; ?>"> <button class="btn btn-xs btn-success">
                                                         ...
                                                         <?php
															 ...
															 if (in_array('VERIFY_MODULES_CONFIGURATION', $roledata) && $row_getinstcourses['added_by']!=$_SESSION['MM_full_names']) 
{
														if($row_getinstcourses['status']=='22')
														{
														?>
                                                    <a href="viewinstitutioncourses.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=11&inst_id=<?php echo $row_getinstcourses['institution_id']; ?>&inst_course_id=<?php echo $row_getinstcourses['inst_course_id']; ?>"> <button class="btn btn-xs btn-warning">
															<i class="ace-icon fa fa-check bigger-120"></i>															</button></a>
												...
												?>                  
                                                  ...
                                                  <?php } while ($row_getinstcourses = mysqli_fetch_assoc($getinstcourses)); ?>
											  </tbody>
											</table>
										</div>
							  </div>
...
mysqli_free_result($getinstcourses);
