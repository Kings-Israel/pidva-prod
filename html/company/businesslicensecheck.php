...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "checkindb")) {
	...
	if (isset($_POST['license_number']) || isset($_POST['license_type'])) {
		...
		$query_getstudent = "SELECT * FROM pel_company_license WHERE (license_number = '" . $license_number . "' OR license_type = '" . $license_type . "') AND expiry_date > '" . $datetoday . "' AND status = '11'";
		...
		if ($totalRows_getstudent > 0) {
			...
			$insertSQL = sprintf(
				"INSERT INTO pel_company_license (company_name, license_number, status, date_added, added_by, expiry_date, data_source, data_notes, license_year, license_type, license_photo, operation_status, country, search_id, registration_date) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
				...
				GetSQLValueString(strtoupper($row_getstudent['license_year']), "text"),
				GetSQLValueString(strtoupper($row_getstudent['license_type']), "text"),
				GetSQLValueString($row_getstudent['license_photo'], "text"),
				GetSQLValueString($row_getstudent['operation_status'], "text"),
			...
			if (mysqli_error($connect)) {
				$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the License Data havent been added.
											<br />
										</div>';
			} else {
				$updateGoTo = "businesslicensecheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
				/* if (isset($_SERVER['QUERY_STRING'])) {
    ...
    $updateGoTo .= $_SERVER['QUERY_STRING'];
			...
			$errorcode = '<div class="alert alert-danger">
										 ...
										 NO Previous Details of License Compliance found
											<br />
										</div>';
		}
	}
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	if (is_uploaded_file($_FILES['license_photo']['tmp_name'])) {
		...
		$a = $_POST['license_type'] . "-" . $_POST['search_id'] . "-" . $date_insert;
		$rawname = $_FILES['license_photo']['name'];
		"Upload: " . $a . "_" . $_FILES["license_photo"]["name"];
		$file = "licensephotos/" . $a . "_" . $_FILES["license_photo"]["name"];

		move_uploaded_file(
			$_FILES["license_photo"]["tmp_name"],

			"licensephotos/" . $a . "_" . $_FILES["license_photo"]["name"]
		);

		$filenameuploaded = $a . "_" . $_FILES["license_photo"]["name"];
	} else {
		$filenameuploaded = $_POST['license_photo2'];
	}
	$updateSQL = sprintf(
		"UPDATE pel_company_license SET company_name=%s, license_number=%s, status=%s, date_added=%s, added_by=%s, expiry_date=%s, data_source=%s, data_notes=%s, license_year=%s, license_type=%s, license_photo=%s, operation_status=%s, country=%s, registration_date=%s WHERE license_id=%s",
		...
		GetSQLValueString($_POST['license_year'], "text"),
		...
		GetSQLValueString($_POST['registration_date'], "text"),
		GetSQLValueString($_POST['license_id'], "int")
	);
	...
	if (mysqli_error($connect)) {
		$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the Business License were not updated succesfully.
											<br />
										</div>';
	} else {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "newdetails")) {
	...
	if (is_uploaded_file($_FILES['license_photo']['tmp_name'])) {
		...
		$insertSQL = sprintf(
			...
			GetSQLValueString($_POST['data_source'], "text"),
			GetSQLValueString($_POST['data_notes'], "text"),
			GetSQLValueString(strtoupper($_POST['license_year']), "text"),
			GetSQLValueString(strtoupper($_POST['license_type']), "text"),
			GetSQLValueString($filenameuploaded, "text"),
			GetSQLValueString($_POST['operation_status'], "text"),
			GetSQLValueString($_POST['country'], "text"),
			GetSQLValueString($_POST['search_id'], "text"),
		...
		if (mysqli_error($connect)) {
			$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the Business License Data havent been added.
											<br />
										</div>';
		} else {
			$updateGoTo = "businesslicensecheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	...
	$deleteSQLreject = sprintf(
		"UPDATE pel_company_license SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s and license_id=%s",
...
if ((isset($_GET['license_id'])) && ($_GET['license_id'] != "")) {
	if ($_GET['status'] == '00') {
		$deleteSQL = sprintf(
			"UPDATE pel_company_license SET status=%s, added_by=%s, date_added=now() WHERE license_id=%s",
			GetSQLValueString($_GET['status'], "text"),
			GetSQLValueString($_GET['fullnames'], "text"),
			GetSQLValueString($_GET['license_id'], "int")
		);
		mysqli_select_db($connect, $database_connect);
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if ($_GET['status'] == '11') {
		...
		$deleteSQL2 = sprintf(
			"UPDATE pel_company_license SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s and license_id=%s",
			GetSQLValueString($_GET['status'], "text"),
...
<head>
	...
	<meta charset="utf-8" />
	<title>Company Business License Details Data Management - Peleza Admin</title>
			...
			<div class="main-content-inner">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Company Business License Details</li>
					</ul><!-- /.breadcrumb -->
...
-->
							<div class="row">
								<div class="col-xs-12">
									...
									<h3 align="left" class="header smaller lighter blue">Add Company Business License Details</h3>
								</div>
								<!--   <div  class="col-xs-6">
								...
								<div>
									...
									$query_getdetails = "SELECT * FROM pel_company_license WHERE search_id = '" . $search_ref . "'";
									...
									?>
										...
										<div class="col-lg-12" align="center">
											<h3 align="left" class=" smaller lighter blue"><strong>BUSINESS LICENSE CHECK DETAILS: </strong>
											</h3>
											<?php
											do {
											?>
												<h4 align="left" class=" smaller lighter blue"><strong>LICENSE TYPE: <?php echo $row_getdetails['license_type']; ?></strong>
												</h4>
												...
												<table id="simple-table" class="table table-bordered table-hover">
													...
													<tr>
														...
														<td class="hidden-480">
															...
															<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['license_id']; ?>" role="button" class="green" data-toggle="modal">
																<button class="btn btn-xs btn-info">
																	<i class="ace-icon fa fa-pencil bigger-120"></i> </button></a>

															<div id="modal-editdetails-<?php echo $row_getdetails['license_id']; ?>" class="modal fade" tabindex="-1">
																	...
																	<div class="modal-content">
																		...
																		<div class="modal-body padding">
																			<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails">
																				...
																				<input type="hidden" id="license_id" name="license_id" value="<?php echo $row_getdetails['license_id']; ?>" />
																				...
																				<div class="col-sm-8"><span id="sprytextfield4">
																						<input type="text" value="<?php echo $row_getdetails['license_number']; ?>" id="license_number" name="license_number" />
																						<span class="textfieldRequiredMsg">*</span></span></div>
																				...
																				<label class="col-sm-4">License Year</label>

																				<div class="col-sm-7"><span id="sprytextfield5">
																						<input type="text" value="<?php echo $row_getdetails['license_year']; ?>" id="license_year" name="license_year" required />
																						<span class="textfieldRequiredMsg">*</span></span></div>
																				...
																				<label class="col-sm-4">License Type</label>

																				<div class="col-sm-7"><span id="sprytextfield5">
																						<input type="text" value="<?php echo $row_getdetails['license_type']; ?>" id="license_type" name="license_type" required />
																						<span class="textfieldRequiredMsg">*</span></span></div>
																				...
																				<label class="col-sm-4">License Status</label>

																				<div class="col-sm-7"><span id="spryselect3">
																						<select class="chosen-select form-control" name="operation_status" id="operation_status" data-placeholder="Choose License Status..." required>
																				...
																				<div class="col-sm-8">
																					Currennt Photo <br />
																					<img width="200px" height="200px" src="licensephotos/<?php echo $row_getdetails['license_photo']; ?>" alt="License Photos">

																					<input id="license_photo2" name="license_photo2" type="hidden" value="<?php echo $row_getdetails['license_photo']; ?>" />
																				</div>
																				<br />
																				...
																				<div class="col-sm-8">
																					<input class="form-control" id="license_photo" name="license_photo" type="file" />
																				</div>
															...
															?>
																<a href="businesslicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=00&license_id=<?php echo $row_getdetails['license_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
																		<i class="ace-icon fa fa-trash-o bigger-120"></i> </button></a> <?php
																																	}
																																	if ($row_getdetails['status'] == '00') {

																																		?>
																<a href="businesslicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=22&license_id=<?php echo $row_getdetails['license_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
													...
													<tr>
														<td colspan="5"><img width="400px" src="licensephotos/<?php echo $row_getdetails['license_photo']; ?>">
														</td>
										...
										<?php
												if ($totalRows_getdetails > 0) {
										?>
											<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="businesslicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=11&search_id_approve=<?php echo $search_ref; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>&license_id=<?php echo $row_getdetails['license_id']; ?>"> <button class="btn btn-xs btn-success">
													<i class="ace-icon fa fa-check bigger-120">Approve</i> </button></a>

											<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-addreject-<?php echo $row_getdetails['license_id']; ?>" role="button" class="blue" data-toggle="modal">
												<button class="btn btn-xs btn-danger"> Reject With Reason </button></a>
										<?php
												}
										?>
										<div id="modal-addreject-<?php echo $row_getdetails['license_id']; ?>" class="modal fade" tabindex="-1">
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header no-padding">
														<div class="table-header">
							...
							<div class="col-lg-12" align="left">
								...
								<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#" role="button" class="orange">
									<button class="btn btn-xs btn-warning">
										<i class="ace-icon smaller-80 green"></i>Fetch From License Organisation
									</button></a>
										...
										<div class="modal-content">
											...
											<div class="modal-body padding">
												<form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="checkindb">
													...
													<label class="col-sm-4">Enter License Type</label>

													<div class="col-sm-8"><input type="text" id="license_type" name="license_type" />
													</div>
										...
										<div class="modal-content">
											...
											<div class="modal-body padding">
												<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="newdetails">
													...
													<div class="col-sm-7"><span id="sprytextfield5">
															<input type="text" id="license_year" name="license_year" required />
															<span class="textfieldRequiredMsg">*</span></span></div>
													...
													<div class="col-sm-7"><span id="spryselect3">
															<select class="chosen-select form-control" name="operation_status" id="operation_status" data-placeholder="Choose Tax Status..." required>
																<!--  <select name="client_industry" id="client_industry">-->
																<option value="">License Status</option>
																<option value="VALID">VALID</option>
																<option value="EXPIRED">EXPIRED</option>
													...
													<div class="col-sm-8">
														<input class="form-control" id="license_photo" name="license_photo" type="file" required />
													</div>
