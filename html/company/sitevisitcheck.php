...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	if (is_uploaded_file($_FILES['building_photo']['tmp_name'])) {
		...
		$file = "sitevisit/" . $a . "_" . $_FILES["building_photo"]["name"];

		move_uploaded_file(
			$_FILES["building_photo"]["tmp_name"],

			"sitevisit/" . $a . "_" . $_FILES["building_photo"]["name"]
		);

		$filenameuploaded = $a . "_" . $_FILES["building_photo"]["name"];
	} else {
		$filenameuploaded = $_POST['building_photo2'];
	}
	$updateSQL = sprintf(
		"UPDATE pel_data_residence SET physical_address=%s, house_number=%s, status=%s, date_added=%s, added_by=%s, street=%s, data_source=%s, data_notes=%s, building_name=%s, building_photo=%s, country=%s WHERE residence_id=%s",
	...
	if (mysqli_error($connect)) {
		...
		$updateGoTo = "sitevisitcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
		/* if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "newdetails")) {
	...
	if (is_uploaded_file($_FILES['building_photo']['tmp_name'])) {
		...
		$insertSQL = sprintf(
			"INSERT INTO pel_data_residence (physical_address, house_number, status, date_added, added_by, street, data_source, data_notes, building_name, building_photo, country, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s)",
			GetSQLValueString(strtoupper($_POST['physical_address']), "text"),
			GetSQLValueString(strtoupper($_POST['house_number']), "text"),
			GetSQLValueString($_POST['status'], "text"),
			GetSQLValueString($_POST['added_by'], "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	...
	$deleteSQLreject = sprintf(
		"UPDATE pel_data_residence SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['residence_id'])) && ($_GET['residence_id'] != "")) {
	if ($_GET['status'] == '00') {
		$deleteSQL = sprintf(
			"UPDATE pel_data_residence SET status=%s, added_by=%s, date_added=now() WHERE residence_id=%s",
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if ($_GET['status'] == '11') {
		...
		$deleteSQL2 = sprintf(
			"UPDATE pel_data_residence SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
			GetSQLValueString($_GET['status'], "text"),
...
<head>
	...
	<meta charset="utf-8" />
	<title>Company Site Visit Details Data Management - Peleza Admin</title>
			...
			<div class="main-content-inner">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Company Site Visit Details</li>
					</ul><!-- /.breadcrumb -->
...
-->
							<div class="row">
								<div class="col-xs-12">
									...
									<h3 align="left" class="header smaller lighter blue">Add Company Site Visit Details</h3>
								</div>
								<!--   <div  class="col-xs-6">
								...
								<div>
									...
									?>
										...
										<div class="col-lg-12" align="center">
											<h3 align="left" class=" smaller lighter blue"><strong>SITE VISIT CHECK DETAILS: </strong>
											</h3>
											...
											<table id="simple-table" class="table table-bordered table-hover">
												...
												<tr>
													<td>
														<a href="#"><?php echo $row_getdetails['building_name']; ?> </a>
													</td>
													...
													<td class="hidden-480">
														...
														<div id="modal-editdetails-<?php echo $row_getdetails['residence_id']; ?>" class="modal fade" tabindex="-1">
																...
																<div class="modal-content">
																	...
																	<div class="modal-body padding">
																		<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails">
																			...
																			<div class="col-sm-8">
																				Current Photo <br />
																				<img width="200px" height="200px" src="sitevisit/<?php echo $row_getdetails['building_photo']; ?>" alt="DL Photos">
														...
														?>
															<a href="sitevisitcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=00&residence_id=<?php echo $row_getdetails['residence_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
																	<i class="ace-icon fa fa-trash-o bigger-120"></i> </button></a> <?php
																																}
																																if ($row_getdetails['status'] == '00') {

																																	?>
															<a href="sitevisitcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=22&residence_id=<?php echo $row_getdetails['residence_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
												...
												<tr>
													<td colspan="4"><img width="400px" src="sitevisit/<?php echo $row_getdetails['building_photo']; ?>">
													</td>
