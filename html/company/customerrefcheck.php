...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	if (is_uploaded_file($_FILES['reference_document']['tmp_name'])) {
		date_default_timezone_set('Africa/Nairobi');
		$date_insert = date('dmYhis');
		$a = $_POST['customer_name'] . "-" . $_POST['search_id'] . "-" . $date_insert;
		$rawname = $_FILES['reference_document']['name'];
		"Upload: " . $a . "_" . $_FILES["reference_document"]["name"];
		$file = "customerreference/" . $a . "_" . $_FILES["reference_document"]["name"];

		move_uploaded_file(
			$_FILES["reference_document"]["tmp_name"],

			"customerreference/" . $a . "_" . $_FILES["reference_document"]["name"]
		);

		$filenameuploaded = $a . "_" . $_FILES["reference_document"]["name"];
	} else {
		$filenameuploaded = $_POST['reference_document2'];
	}
	$updateSQL = sprintf(
		"UPDATE pel_company_customer_ref SET contact_person_name=%s, customer_name=%s, status=%s, date_added=%s, added_by=%s, contact_person_title=%s, data_source=%s, data_notes=%s, reference_contact=%s, reference_document=%s WHERE customer_ref_id=%s",
		GetSQLValueString(strtoupper($_POST['contact_person_name']), "text"),
		GetSQLValueString(strtoupper($_POST['customer_name']), "text"),
		GetSQLValueString($_POST['status'], "text"),
		GetSQLValueString($_POST['date_added'], "text"),
		GetSQLValueString($_POST['added_by'], "text"),
		GetSQLValueString($_POST['contact_person_title'], "text"),
		GetSQLValueString($_POST['data_source'], "text"),
		GetSQLValueString($_POST['data_notes'], "text"),
		GetSQLValueString($_POST['reference_contact'], "text"),
		GetSQLValueString($_POST['license_type'], "text"),
		GetSQLValueString($filenameuploaded, "text"),
		GetSQLValueString($_POST['customer_ref_id'], "int")
	);
	...
	if (mysqli_error($connect)) {
		$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the Reference were not updated succesfully.
											<br />
										</div>';
	} else {

		$updateGoTo = "customerrefcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
		/* if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "newdetails")) {
	...
	$insertSQL = sprintf(
		"INSERT INTO pel_company_customer_ref (contact_person_name, customer_name, status, date_added, added_by, contact_person_title, data_source, data_notes, reference_contact, reference_document, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s)",
		GetSQLValueString(strtoupper($_POST['contact_person_name']), "text"),
		GetSQLValueString(strtoupper($_POST['customer_name']), "text"),
		GetSQLValueString($_POST['status'], "text"),
		GetSQLValueString($_POST['added_by'], "text"),
		...
		GetSQLValueString($_POST['data_source'], "text"),
		GetSQLValueString($_POST['data_notes'], "text"),
		GetSQLValueString(strtoupper($_POST['reference_contact']), "text"),
		GetSQLValueString($filenameuploaded, "text"),
	...
	if (mysqli_error($connect)) {
		$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the Reference Data havent been added.
											<br />
										</div>';
	} else {
		$updateGoTo = "customerrefcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	...
	$deleteSQLreject = sprintf(
		"UPDATE pel_company_customer_ref SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s and customer_ref_id=%s",
...
if ((isset($_GET['customer_ref_id'])) && ($_GET['customer_ref_id'] != "")) {
	if ($_GET['status'] == '00') {
		$deleteSQL = sprintf(
			"UPDATE pel_company_customer_ref SET status=%s, added_by=%s, date_added=now() WHERE customer_ref_id=%s",
			GetSQLValueString($_GET['status'], "text"),
			GetSQLValueString($_GET['fullnames'], "text"),
			GetSQLValueString($_GET['customer_ref_id'], "int")
		);
		mysqli_select_db($connect, $database_connect);
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if ($_GET['status'] == '11') {
		...
		$deleteSQL2 = sprintf(
			"UPDATE pel_company_customer_ref SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s and customer_ref_id=%s",
			GetSQLValueString($_GET['status'], "text"),
...
<head>
	...
	<meta charset="utf-8" />
	<title>Company Customer Reference Details Data Management - Peleza Admin</title>
			...
			<div class="main-content-inner">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Company Customer Reference Details</li>
					</ul><!-- /.breadcrumb -->
...
-->
							<div class="row">
								<div class="col-xs-12">
									...
									<h3 align="left" class="header smaller lighter blue">Add Company Customer Reference Details</h3>
								</div>
								<!--   <div  class="col-xs-6">
								...
								<div>
									...
									$query_getdetails = "SELECT * FROM pel_company_customer_ref WHERE search_id = '" . $search_ref . "'";
									...
									?>
										...
										<div class="col-lg-12" align="center">
											<h3 align="left" class=" smaller lighter blue"><strong>CUSTOMER REFERENCE CHECK DETAILS: </strong>
											</h3>
											<?php
											do {
											?>
												...
												<table id="simple-table" class="table table-bordered table-hover">
													...
													<tr>
														<td>
															<a href="#"><?php echo $row_getdetails['contact_person_name']; ?> </a>
														</td>
														<td><?php echo $row_getdetails['customer_name']; ?></td>
														<td><?php echo $row_getdetails['contact_person_title']; ?></td>
														<td><?php echo $row_getdetails['reference_contact']; ?></td>
														...
														<td class="hidden-480">
															...
															<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['customer_ref_id']; ?>" role="button" class="green" data-toggle="modal">
																<button class="btn btn-xs btn-info">
																	<i class="ace-icon fa fa-pencil bigger-120"></i> </button></a>

															<div id="modal-editdetails-<?php echo $row_getdetails['customer_ref_id']; ?>" class="modal fade" tabindex="-1">
																	...
																	<div class="modal-content">
																		...
																		<div class="modal-body padding">
																			<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails">
																				...
																				<input type="hidden" id="customer_ref_id" name="customer_ref_id" value="<?php echo $row_getdetails['customer_ref_id']; ?>" />
																				...
																				<label class="col-sm-4">Contact Person Name</label>

																				<div class="col-sm-8"><span id="sprytextfield1">
																						<input type="text" id="contact_person_name" name="contact_person_name" value="<?php echo $row_getdetails['contact_person_name']; ?>" required />
																				...
																				<div class="col-sm-8"><span id="sprytextfield4">
																						<input type="text" value="<?php echo $row_getdetails['customer_name']; ?>" id="customer_name" name="customer_name" />
																						<span class="textfieldRequiredMsg">*</span></span></div>

																				<br />
																				<div class="space-10"></div>

																				<label class="col-sm-4">Contact Person Title</label>

																				<div class="col-sm-8"><span id="sprytextfield4">
																						<input type="text" value="<?php echo $row_getdetails['contact_person_title']; ?>" id="contact_person_title" name="contact_person_title" required />
																						<span class="textfieldRequiredMsg">*</span></span></div>
																				<br />
																				<div class="space-10"></div>

																				<label class="col-sm-4">Reference Contact</label>

																				<div class="col-sm-7"><span id="sprytextfield5">
																						<input type="text" value="<?php echo $row_getdetails['reference_contact']; ?>" id="reference_contact" name="reference_contact" required />
																						<span class="textfieldRequiredMsg">*</span></span></div>
																				...
																				<label class="col-sm-4">Reference Document</label>

																				<div class="col-sm-8">
																					Currennt Photo <br />
																					<img width="200px" height="200px" src="customerreference/<?php echo $row_getdetails['reference_document']; ?>" alt="License Photos">

																					<input id="reference_document2" name="reference_document2" type="hidden" value="<?php echo $row_getdetails['reference_document']; ?>" />
																				</div>
																				<br />
																				...
																				<div class="col-sm-8">
																					<input class="form-control" id="reference_document" name="reference_document" type="file" />
																				</div>
															...
															?>
																<a href="customerrefcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=00&customer_ref_id=<?php echo $row_getdetails['customer_ref_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
																		<i class="ace-icon fa fa-trash-o bigger-120"></i> </button></a> <?php
																																	}
																																	if ($row_getdetails['status'] == '00') {

																																		?>
																<a href="customerrefcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=22&customer_ref_id=<?php echo $row_getdetails['customer_ref_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
													...
													<tr>
														<td colspan="5"><img width="400px" src="customerreference/<?php echo $row_getdetails['reference_document']; ?>">
														</td>
										...
										?>
										<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="customerrefcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=11&search_id_approve=<?php echo $search_ref; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>&customer_ref_id=<?php echo $row_getdetails['customer_ref_id']; ?>"> <button class="btn btn-xs btn-success">
												<i class="ace-icon fa fa-check bigger-120">Approve</i> </button></a>

										<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-addreject-<?php echo $row_getdetails['customer_ref_id']; ?>" role="button" class="blue" data-toggle="modal">
											<button class="btn btn-xs btn-danger"> Reject With Reason </button></a>
										<div id="modal-addreject-<?php echo $row_getdetails['customer_ref_id']; ?>" class="modal fade" tabindex="-1">
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-header no-padding">
														<div class="table-header">
										...
										<div class="modal-content">
											...
											<div class="modal-body padding">
												<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="newdetails">
													...
													<label class="col-sm-4">Customer Title</label>

													<div class="col-sm-8"><span id="sprytextfield4">
															<input type="text" id="contact_person_title" name="contact_person_title" required />
															<span class="textfieldRequiredMsg">*</span></span></div>
													<br />
													<div class="space-10"></div>
