...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "checkindb")) {
	...
	if (isset($_POST['registration_number']) || isset($_POST['tax_number'])) {
		...
		$query_getstudent = "SELECT * FROM pel_company_tax_data WHERE (registration_number = '" . $registration_number . "' OR tax_number = '" . $tax_number . "') AND expiry_date > '" . $datetoday . "' AND status = '11'";
		...
		if ($totalRows_getstudent > 0) {
			...
			$insertSQL = sprintf(
				"INSERT INTO pel_company_tax_data (company_name, registration_number, status, date_added, added_by, expiry_date, data_source, data_notes, tax_organisation, tax_number, tax_photo, tax_status, country, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	if (is_uploaded_file($_FILES['tax_photo']['tmp_name'])) {
		...
		move_uploaded_file(
			$_FILES["tax_photo"]["tmp_name"],

			"taxcompliance/" . $a . "_" . $_FILES["tax_photo"]["name"]
		);

		$filenameuploaded = $a . "_" . $_FILES["tax_photo"]["name"];
	} else {
		$filenameuploaded = $_POST['tax_photo2'];
	}
	$updateSQL = sprintf(
		"UPDATE pel_company_tax_data SET company_name=%s, registration_number=%s, status=%s, date_added=%s, added_by=%s, expiry_date=%s, data_source=%s, data_notes=%s, tax_organisation=%s, tax_number=%s, tax_photo=%s, tax_status=%s, country=%s WHERE tax_compliance_id=%s",
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	...
	$deleteSQLreject = sprintf(
		"UPDATE pel_company_tax_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['tax_compliance_id'])) && ($_GET['tax_compliance_id'] != "")) {
	if ($_GET['status'] == '00') {
		$deleteSQL = sprintf(
			"UPDATE pel_company_tax_data SET status=%s, added_by=%s, date_added=now() WHERE tax_compliance_id=%s",
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if ($_GET['status'] == '11') {
		...
		$deleteSQL2 = sprintf(
			"UPDATE pel_company_tax_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
			GetSQLValueString($_GET['status'], "text"),
...
<head>
	...
	<meta charset="utf-8" />
	<title>Company Tax Compliance Details Data Management - Peleza Admin</title>
			...
			<div class="main-content-inner">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Company Tax Compliance Details</li>
					</ul><!-- /.breadcrumb -->
...
-->
							<div class="row">
								<div class="col-xs-12">
									...
									<h3 align="left" class="header smaller lighter blue">Add Company Tax Details</h3>
								</div>
								<!--   <div  class="col-xs-6">
								...
								<div>
									...
									$query_getdetails = "SELECT * FROM pel_company_tax_data WHERE search_id = '" . $search_ref . "'";
									...
									?>
										...
										<div class="col-lg-12" align="center">
											...
											<table id="simple-table" class="table table-bordered table-hover">
																	...
																	<div class="modal-body padding">
																		<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails">
																			...
																			<div class="col-sm-8"><span id="sprytextfield4">
																					<input type="text" value="<?php echo $row_getdetails['registration_number']; ?>" id="registration_number" name="registration_number" />
																					<span class="textfieldRequiredMsg">*</span></span></div>
																			...
																			<div class="col-sm-8">
																				Currennt Photo <br />
																				<img width="200px" height="200px" src="taxcompliance/<?php echo $row_getdetails['tax_photo']; ?>" alt="Tax Photos">
												...
												<tr>
													<td colspan="5"><img width="400px" src="taxcompliance/<?php echo $row_getdetails['tax_photo']; ?>">
													</td>
