...
$rawname = $_FILES['photo']['name'];
    ...
    $prefix = "photo";
    $filenameuploaded = uploadFile($prefix,"watchlistphotos",$a."_".$_FILES[$prefix]["name"]);
...
$filenameuploaded = $_POST['photo2']; 
  ...
  $updateSQL = sprintf("UPDATE pel_watchlist_individual_data SET first_name=%s, status=%s, identity_number=%s, added_by=%s, search_id=%s, date_of_birth=%s, country=%s, citizenship=%s, data_source=%s, data_notes=%s, watchlist_status=%s, father_name=%s, date_added=now(), photo=%s WHERE watchlist_id=%s",
...
else
{
  ...
  $updateGoTo = "watchlistindividualcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
$insertSQL = sprintf("INSERT INTO pel_watchlist_individual_data (first_name, status, identity_number, added_by, search_id, date_of_birth, country, citizenship, data_source, data_notes, watchlist_status, father_name, date_added, photo) VALUES (%s, %s, %s,%s, %s, %s, %s, %s,  %s, %s, %s, %s, now(), %s)",
                       GetSQLValueString(strtoupper($_POST['first_name']), "text"),
					   GetSQLValueString("22", "text"),
                       GetSQLValueString(strtoupper($_POST['identity_number']), "text"),
                       GetSQLValueString($_POST['added_by'], "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_watchlist_individual_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_watchlist_individual_data SET status=%s, added_by=%s, date_added=now() WHERE watchlist_id=%s",
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_watchlist_individual_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Individual Watchlist Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Individual Watchlist Details</li>
						</ul><!-- /.breadcrumb -->
...
<div class="row">
									<div class="col-xs-12">
									  ...
									  <h3 align="left" class="header smaller lighter blue">Add Individual Watchlist Details</h3>
                                      </div>
                                         <!--   <div  class="col-xs-6">
...
mysqli_select_db($connect,$database_connect);
echo $query_getfiles2 = "SELECT pel_psmt_request.request_id, pel_psmt_files.psmtfile_filetoken,pel_psmt_files.psmtfile_id,pel_psmt_files.psmtfile_name,pel_psmt_files.psmtfile_type,pel_psmt_files.`status`,
...
if($totalRows_getfiles2>0)	  
{
                              ...
                              do
 {
  ?>	
    <tr>
                                  <td width="35%"><b><?php echo $row_getfiles2['psmtfile_type']; ?>:</b></td>
                                  <td><?php echo $row_getfiles2['psmtfile_name']; ?></td>
                                </tr>

   <?php
 } while ($row_getfiles2 = mysqli_fetch_assoc($getfiles2)); ?> </table>	
...
$query_getdetails = "SELECT * FROM pel_watchlist_individual_data WHERE search_id = '".$search_ref."' and module_name ='watchlistindividualcheck'";
...
if($totalRows_getdetails>0)
{
   ...
   <div id="modal-editdetails-<?php echo $row_getdetails['watchlist_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
...
$totalRows_getcountries = mysqli_num_rows($getcountries);
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="watchlistphotos/<?php echo $row_getdetails['photo']; ?>" alt="ID Photos">
...
<button class="btn" type="reset">
                                ...
                                <?php
                                	if($row_getdetails['status']=='11' || $row_getdetails['status']=='22')
														{
														?>
                                                        <a href="watchlistindividualcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&watchlist_id=<?php echo $row_getdetails['watchlist_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
                                	...
                                	if($row_getdetails['status']=='00')
														...
														?>
                                                    <a href="watchlistindividualcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&watchlist_id=<?php echo $row_getdetails['watchlist_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
...
<?php
                                               ...
                                               <thead> 
                                                ...
                                                <strong>PHOTO:</strong>
                                                    ...
                                                    <td  colspan="6">
                                                <strong>IDENTITY PHOTO:</strong>
                                                        </td> 
                                                    
                                                          </tr></thead>
                                                    <tr>
                                                    <td colspan="5"><img width="400px" src="individualphotos/<?php echo $row_getdetails['identity_holder_photo']; ?>">
                                   </td>
                                                      <td colspan="6"><img width="400px" src="watchlistphotos/<?php echo $row_getdetails['photo']; ?>">
                                   </td></tr>
                                    <thead> 
                                                 	 	<tr>	  <td  colspan="11">
                                                <strong>NOTES:</strong>
