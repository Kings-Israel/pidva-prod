...
$query_getstudent = "SELECT * FROM pel_individual_fprint_data WHERE identity_number = '".$identity_number."' AND status = '11'";
...
if($totalRows_getstudent > 0)
{
 ...
 $insertSQL = sprintf("INSERT INTO pel_individual_fprint_data (first_name, identity_number, status, added_by, data_source, data_notes, date_added, search_id, finger_print_pel, country,finger_print_src, match_status) VALUES (%s, %s,%s, %s, %s, %s, now(),  %s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString($row_getstudent['finger_print_pel'], "text"),
					   GetSQLValueString(strtoupper($row_getstudent['country']), "text"),
					   GetSQLValueString($row_getstudent['finger_print_src'], "text"),
					   GetSQLValueString($row_getstudent['match_status'], "text"));
...
$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the Finger Print Data havent been added.
											<br />
										</div>';
...
{ 
  $updateGoTo = "fingerprintcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
else
{
	$errorcode = '<div class="alert alert-danger">
										 ...
										 NO Previous Details of FInger Print found
											<br />
										</div>';
...
if (is_uploaded_file($_FILES['finger_print_pel']['tmp_name'])) {
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = "PEL-TAKEN-".$_POST['identity_number']."-".$_POST['search_id']."-".$date_insert; 
$rawname = $_FILES['finger_print_pel']['name'];
 	  "Upload: ".$a."_". $_FILES["finger_print_pel"]["name"];
	  $file="fingerprint/".$a."_". $_FILES["finger_print_pel"]["name"];

    require_once "../../uploads.php";
    $prefix = "finger_print_pel";
    $filenameuploaded = uploadFile($prefix,"individual-fingerprint",$a."_".$_FILES[$prefix]["name"]);


}
else
{
$filenameuploaded = $_POST['finger_print_pel2']; 

}


if (is_uploaded_file($_FILES['finger_print_src']['tmp_name'])) {
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = "SRC-TAKEN-".$_POST['identity_number']."-".$_POST['search_id']."-".$date_insert; 

$rawname = $_FILES['finger_print_src']['name'];
 	  "Upload: ".$a."_". $_FILES["finger_print_src"]["name"];
	  $file="fingerprint/".$a."_". $_FILES["finger_print_src"]["name"];
	
    require_once "../../uploads.php";
    $prefix = "finger_print_src";
    $filenameuploaded_individual = uploadFile($prefix,"individual-fingerprint",$a."_".$_FILES[$prefix]["name"]);

}
else
{
$filenameuploaded_individual = $_POST['finger_print_src2']; 
}


  $updateSQL = sprintf("UPDATE pel_individual_fprint_data SET first_name=%s, identity_number=%s, status=%s, added_by=%s, data_source=%s, data_notes=%s, date_added=now(), search_id=%s, finger_print_pel=%s, country=%s, finger_print_src=%s, match_status=%s WHERE fprint_id=%s",
					   ...
					   GetSQLValueString($_POST['match_status'], "text"),
					   GetSQLValueString($_POST['fprint_id'], "text"));
...
$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the Finger Print were not updated succesfully.
											<br />
										</div>';
...
$insertSQL = sprintf("INSERT INTO pel_individual_fprint_data (identity_type, identity_number, status, added_by, date_added, search_id, country, module_name) VALUES (%s, %s, %s,  %s, now(), %s, %s, %s)",
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_individual_fprint_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['fprint_id'])) && ($_GET['fprint_id'] != "")) {
if($_GET['status']=='000')
{
  $deleteSQL = sprintf("UPDATE pel_individual_fprint_data SET status=%s, added_by=%s, date_added=now() WHERE fprint_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
					   GetSQLValueString($_GET['fullnames'], "text"),
					    GetSQLValueString($_GET['fprint_id'], "int"));
					   mysqli_select_db($connect,$database_connect);
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_individual_fprint_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Individual Finger Print Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Individual Finger Print Details</li>
						</ul><!-- /.breadcrumb -->
...
<div class="row">
									<div class="col-xs-12">
									  ...
									  <h3 align="left" class="header smaller lighter blue">Add Individual Finger Print Details</h3>
                                      </div>
                                         <!--   <div  class="col-xs-6">
...
$query_getdetails = "SELECT * FROM pel_individual_fprint_data WHERE search_id = '".$search_ref."'";
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>INDIVIDUAL FINGER PRINT CHECK DETAILS: </strong> 
</h3>
										...
										<thead>
                                                 ...
                                                 <td><?php echo $row_getdetails['country']; ?></td>
                                                   <td><?php echo $row_getdetails['match_status']; ?></td>
	...
	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['fprint_id']; ?>" role="button" class="green" data-toggle="modal">	
                                 <button class="btn btn-xs btn-info">  
															<i class="ace-icon fa fa-pencil bigger-120"></i>															</button></a>

   <div id="modal-editdetails-<?php echo $row_getdetails['fprint_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
 ...
 <input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
										<input type="hidden" id="fprint_id" name="fprint_id" value="<?php echo $row_getdetails['fprint_id']; ?>"/>
						...
						<br/>
                               	...
                               	<br/>
                                	<div class="space-10"></div>
																...
																<label class="col-sm-4">Match Status</label>

																<div class="col-sm-8">
															  <select class="chosen-select form-control"  name="match_status" id="match_status"  data-placeholder="Choose match_status..." required>
														    <!--  <select name="client_industry" id="client_industry">-->
                                                           <option  value="<?php echo $row_getdetails['match_status']; ?>"><?php echo $row_getdetails['match_status']; ?></option>
...
$totalRows_getcountries2 = mysqli_num_rows($getcountries2);
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Peleza Taken Finger Print</label>

														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="<?php echo $row_getdetails['finger_print_pel']; ?>" alt="Finger Print Peleza Photos">
                                                                 
                                                  <input id="finger_print_pel2" name="finger_print_pel2" type="hidden" value="<?php echo $row_getdetails['finger_print_pel']; ?>"/>    
											  </div>
                                               	<br/>
															...
															<div class="space-10"></div> 
                                                               <label class="col-sm-4">Click to Change Image</label>
                                                                <div class="col-sm-8">
															          <input class="form-control"  id="finger_print_pel" name="finger_print_pel" type="file"/>
																</div>
							<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Source Photo</label>

														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="<?php echo $row_getdetails['finger_print_src']; ?>" alt="ID Photos">
                                                                 
                                                  <input id="finger_print_src2" name="finger_print_src2" type="hidden" value="<?php echo $row_getdetails['finger_print_src']; ?>"/>    
											  </div>
                                               	<br/>
															...
															<div class="space-10"></div> 
                                                               <label class="col-sm-4">Click to Change Image</label>
                                                                <div class="col-sm-8">
															          <input class="form-control"  id="finger_print_src" name="finger_print_src" type="file"/>
																</div>
							  <br/>				
                                    	<div class="space-10"></div>
...
<button class="btn" type="reset">
                                ...
                                <?php
                                	if($row_getdetails['status']=='11' || $row_getdetails['status']=='22')
														{
														?>
                                                        <a href="fingerprintcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&fprint_id=<?php echo $row_getdetails['fprint_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
                                	...
                                	if($row_getdetails['status']=='00')
														...
														?>
                                                    <a href="fingerprintcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&fprint_id=<?php echo $row_getdetails['fprint_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
...
<?php
                                               ...
                                               <thead> 
                                                 	 	<tr>	  <td  colspan="5">
                                                <strong>FINGER PRINT PELEZA:</strong>
                                                        </td> 				
                                                    <td  colspan="6">
                                                <strong>SOURCE FINGER PRINT:</strong>
                                                        </td> 
                                                    
                                                          </tr></thead>
                                                    <tr>
                                                    <td colspan="5"><img width="400px" src="<?php echo $row_getdetails['finger_print_pel']; ?>">
                                   </td>
                                                      <td colspan="6"><img width="400px" src="<?php echo $row_getdetails['finger_print_src']; ?>">
                                   </td></tr>
                                    <thead> 
                                                 	 	<tr>	  <td  colspan="11">
                                                <strong>NOTES:</strong>
						...
						<br/>
                               	...
                               	<br/>
                                	<div class="space-10"></div>
																...
																<label class="col-sm-4">match_status</label>

																<div class="col-sm-8">
															  <select class="chosen-select form-control"  name="match_status" id="match_status"  data-placeholder="Choose match status..." required>
														    <!--  <select name="client_industry" id="client_industry">-->
...
$totalRows_getcountries2 = mysqli_num_rows($getcountries2);
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Peleza Fingerprint Photo</label>
							...
							<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Source Finger Print</label>
