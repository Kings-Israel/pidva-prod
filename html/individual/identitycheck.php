...
$query_getstudent = "SELECT * FROM pel_individual_id WHERE identity_number = '".$identity_number."' AND status = '11' and identity_type = '".$identity_type."'";
...
if($totalRows_getstudent > 0)
{
 ...
 $insertSQL = sprintf("INSERT INTO pel_individual_id (identity_type, identity_name, identity_number, status, date_of_birth, added_by, gender, data_source, data_notes, date_added, search_id, identity_photo, citizenship, district, identity_country,identity_holder_photo, module_name) VALUES (%s, %s, %s,%s, %s, %s, %s, %s,  %s, now(), %s, %s, %s, %s, %s, %s, %s)",
					   GetSQLValueString(strtoupper($row_getstudent['identity_type']), "text"),
                       GetSQLValueString(strtoupper($row_getstudent['identity_name']), "text"),
                       GetSQLValueString(strtoupper($row_getstudent['identity_number']), "text"),
                       GetSQLValueString("22", "text"),
					   GetSQLValueString($row_getstudent['date_of_birth'], "text"),
                       GetSQLValueString($_POST['added_by'], "text"),
                       GetSQLValueString(strtoupper($row_getstudent['gender']), "text"),
					   ...
					   GetSQLValueString($row_getstudent['identity_photo'], "text"),
					   GetSQLValueString(strtoupper($row_getstudent['citizenship']), "text"),
					   GetSQLValueString(strtoupper($row_getstudent['district']), "text"),
					   GetSQLValueString(strtoupper($row_getstudent['identity_country']), "text"),
					   GetSQLValueString($row_getstudent['identity_holder_photo'], "text"),
					   GetSQLValueString("identitycheck", "text"));
...
$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the ID Data havent been added.
											<br />
										</div>';
...
{ 
  $updateGoTo = "identitycheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
else
{
	$errorcode = '<div class="alert alert-danger">
										 ...
										 NO Previous Details of ID found
											<br />
										</div>';
...
if (is_uploaded_file($_FILES['identity_photo']['tmp_name'])) {
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = $_POST['identity_number']."-".$_POST['search_id']."-".$date_insert; 
$rawname = $_FILES['identity_photo']['name'];
 	  "Upload: ".$a."_". $_FILES["identity_photo"]["name"];
	  $file="identityphotos/".$a."_". $_FILES["identity_photo"]["name"];
	
	   move_uploaded_file($_FILES["identity_photo"]["tmp_name"],

      "identityphotos/".$a."_". $_FILES["identity_photo"]["name"]);

 $filenameuploaded = $a."_".$_FILES["identity_photo"]["name"];


}
else
{
$filenameuploaded = $_POST['identity_photo2']; 

}


if (is_uploaded_file($_FILES['identity_holder_photo']['tmp_name'])) {
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = $_POST['identity_number']."-".$_POST['search_id']."-".$date_insert; 

$rawname = $_FILES['identity_holder_photo']['name'];
 	  "Upload: ".$a."_". $_FILES["identity_holder_photo"]["name"];
	  $file="individualphotos/".$a."_". $_FILES["identity_holder_photo"]["name"];
	
	   move_uploaded_file($_FILES["identity_holder_photo"]["tmp_name"],

      "individualphotos/".$a."_". $_FILES["identity_holder_photo"]["name"]);

$filenameuploaded_individual = $a."_".$_FILES["identity_holder_photo"]["name"];


}
else
{
$filenameuploaded_individual = $_POST['identity_holder_photo2']; 
}


  $updateSQL = sprintf("UPDATE pel_individual_id SET identity_type=%s, identity_name=%s, identity_number=%s, status=%s, date_of_birth=%s, added_by=%s, gender=%s, data_source=%s, data_notes=%s, date_added=now(), search_id=%s, identity_photo=%s, citizenship=%s, district=%s, identity_country=%s,identity_holder_photo=%s WHERE identity_id=%s",
                       GetSQLValueString(strtoupper($_POST['identity_type']), "text"),
                       GetSQLValueString(strtoupper($_POST['identity_name']), "text"),
                       GetSQLValueString(strtoupper($_POST['identity_number']), "text"),
                       GetSQLValueString("22", "text"),
					   GetSQLValueString($_POST['date_of_birth'], "text"),
                       GetSQLValueString($_POST['added_by'], "text"),
                       GetSQLValueString(strtoupper($_POST['gender']), "text"),
					   ...
					   GetSQLValueString(strtoupper($_POST['district']), "text"),
					   GetSQLValueString(strtoupper($_POST['identity_country']), "text"),
					   GetSQLValueString($filenameuploaded_individual, "text"),
...
$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the Identity were not updated succesfully.
											<br />
										</div>';
...
$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the Identity Data havent been added.
											<br />
										</div>';
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "checkiniprs")) {

 
$insertSQL = sprintf("INSERT INTO pel_individual_id (identity_type, identity_number, status, added_by, date_added, search_id, identity_country, module_name) VALUES (%s, %s, %s,  %s, now(), %s, %s, %s)",
                       GetSQLValueString(strtoupper($_POST['identity_type']), "text"),
                       GetSQLValueString(strtoupper($_POST['identity_number']), "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_individual_id SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['identity_id'])) && ($_GET['identity_id'] != "")) {
if($_GET['status']=='000')
{
  $deleteSQL = sprintf("UPDATE pel_individual_id SET status=%s, added_by=%s, date_added=now() WHERE identity_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
					   GetSQLValueString($_GET['fullnames'], "text"),
					    GetSQLValueString($_GET['identity_id'], "int"));
					   mysqli_select_db($connect,$database_connect);
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_individual_id SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
$deleteSQL4 = sprintf("UPDATE pel_psmt_request SET cron_status=%s WHERE request_ref_number=%s",
            GetSQLValueString('22', "text"),
            GetSQLValueString($_GET['search_id_approve'], "text"));
        mysqli_select_db($connect, $database_connect);
        $Result4 = mysqli_query_ported($deleteSQL4, $connect) or die(mysqli_error($connect));
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Individual Identity Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Individual Identity Details</li>
						</ul><!-- /.breadcrumb -->
...
<div class="row">
									<div class="col-xs-12">
									  ...
									  <h3 align="left" class="header smaller lighter blue">Add Individual Identity Details</h3>
                                      </div>
                                         <!--   <div  class="col-xs-6">
...
$query_getdetails = "SELECT * FROM pel_individual_id WHERE search_id = '".$search_ref."' and module_name ='identitycheck'";
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>INDIVIDUAL IDENTITY CHECK DETAILS: </strong> 
</h3>
										...
										<thead>
													<tr>									
                                                      ...
                                                      <th>IDENTITY NAME:</th>
                                                   	...
                                                   	<tr>
                                                    	<td>
                                                   <a href="#"><?php echo $row_getdetails['identity_type']; ?> </a>																								</td><td>
                                                   <a href="#"><?php echo $row_getdetails['identity_name']; ?> </a>																								</td>
                                                   <td><?php echo $row_getdetails['identity_number']; ?></td> 
                                                   <td><?php echo $row_getdetails['date_of_birth']; ?></td>
                                                   <td><?php echo $row_getdetails['gender']; ?></td>  
                                                   <td><?php echo $row_getdetails['identity_country']; ?></td>
                                                   <td><?php echo $row_getdetails['citizenship']; ?></td>
                                                    <td><?php echo $row_getdetails['district']; ?></td>  
	...
	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['identity_id']; ?>" role="button" class="green" data-toggle="modal">	
                                 <button class="btn btn-xs btn-info">  
															<i class="ace-icon fa fa-pencil bigger-120"></i>															</button></a>

   <div id="modal-editdetails-<?php echo $row_getdetails['identity_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
 ...
 <input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
										<input type="hidden" id="identity_id" name="identity_id" value="<?php echo $row_getdetails['identity_id']; ?>"/>
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
           						...
           						<div class="space-10"></div>
																...
																<div class="col-sm-8">
															  <select class="chosen-select form-control"  name="identity_type" id="identity_type"  data-placeholder="Choose Type..." required>
														    <!--  <select name="client_industry" id="client_industry">-->
                                                           <option  value="<?php echo $row_getdetails['identity_type']; ?>"><?php echo $row_getdetails['identity_type']; ?></option>
						...
						<br/>
																...
																<div class="col-sm-8"><span id="sprytextfield1">
																  <input type="text" id="identity_name" name="identity_name" value="<?php echo $row_getdetails['identity_name']; ?>"  required/>
						...
						<br/>
														...
														<label class="col-sm-4">Date of Birth</label>
															  ...
															  <div class="col-sm-8"><span id="sprytextfield4">
															    <input  type="text" value="<?php echo $row_getdetails['date_of_birth']; ?>" id="date_of_birth" name="date_of_birth"  required/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
                               	...
                               	<br/>
                                	<div class="space-10"></div>
																...
																<div class="col-sm-8">
															  <select class="chosen-select form-control"  name="gender" id="gender"  data-placeholder="Choose Gender..." required>
														    <!--  <select name="client_industry" id="client_industry">-->
                                                           <option  value="<?php echo $row_getdetails['gender']; ?>"><?php echo $row_getdetails['gender']; ?></option>
                            ...
                            <br/>	<div class="space-10"></div>
														...
														<label class="col-sm-4">Country</label>
															...
															<select class="chosen-select form-control" name="identity_country" id="identity_country" data-placeholder="Choose Country...">
														 <!--     <select name="client_country" id="client_country">-->
														        <option value="<?php echo $row_getdetails['identity_country']; ?>"><?php echo $row_getdetails['identity_country']; ?></option>
...
$query_getcountries = "SELECT * FROM pel_countries ORDER BY country_nationality ASC";
...
$totalRows_getcountries = mysqli_num_rows($getcountries);
																 ...
																 do { ?> 
                                                                 <option value="<?php echo $row_getcountries['country_nationality']; ?>"><?php echo $row_getcountries['country_nationality']; ?></option> 
								...
								<br/><br/>
														...
														<label class="col-sm-4">District</label>
															  ...
															  <div class="col-sm-7">
															    <input  value="<?php echo $row_getdetails['district']; ?>" type="text" id="district" name="district"/>
														   </div>
					<br/>
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Identity Photo</label>

														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="identityphotos/<?php echo $row_getdetails['identity_photo']; ?>" alt="ID Photos">
                                                                 
                                                  <input id="identity_photo2" name="identity_photo2" type="hidden" value="<?php echo $row_getdetails['identity_photo']; ?>"/>    
											  </div>
                                               	<br/>
															...
															<div class="space-10"></div> 
                                                               <label class="col-sm-4">Click to Change Image</label>
                                                                <div class="col-sm-8">
															          <input class="form-control"  id="identity_photo" name="identity_photo" type="file"/>
																</div>
							<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Identity Holder Photo</label>

														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="individualphotos/<?php echo $row_getdetails['identity_holder_photo']; ?>" alt="ID Photos">
                                                                 
                                                  <input id="identity_holder_photo2" name="identity_holder_photo2" type="hidden" value="<?php echo $row_getdetails['identity_holder_photo']; ?>"/>    
											  </div>
                                               	<br/>
															...
															<div class="space-10"></div> 
                                                               <label class="col-sm-4">Click to Change Image</label>
                                                                <div class="col-sm-8">
															          <input class="form-control"  id="identity_holder_photo" name="identity_holder_photo" type="file"/>
																</div>
							  <br/>				
                                    	<div class="space-10"></div>
		...
		<textarea name="data_notes" id="editorCopy2" required style="display:none;"></textarea>
	</div>
...
<button class="btn" type="reset">
                                ...
                                <?php
                                	if($row_getdetails['status']=='11' || $row_getdetails['status']=='22')
														{
														?>
                                                        <a href="identitycheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&identity_id=<?php echo $row_getdetails['identity_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
                                	...
                                	if($row_getdetails['status']=='00')
														...
														?>
                                                    <a href="identitycheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&identity_id=<?php echo $row_getdetails['identity_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
...
<?php
                                               ...
                                               <thead> 
                                                ...
                                                <strong>PHOTO:</strong>
                                                    ...
                                                    <td  colspan="3">
                                                <strong>UPLOADED IDENTITY PHOTO:</strong>
                                                        </td> 
                                                       <td  colspan="5">
                                                <strong>SOURCE IDENTITY PHOTO:</strong>
                                                        </td> 
                                                          </tr></thead>
                                                    <tr>
                                                    <td colspan="3"><img width="300px" src="individualphotos/<?php echo $row_getdetails['identity_holder_photo']; ?>">
                                   </td>
                                                      <td colspan="3"><img width="300px" src="identityphotos/<?php echo $row_getdetails['identity_photo']; ?>">
                                   </td>     <td colspan="5"><img src="http://46.101.16.235/pidva/html/searches/photos/<?php echo $row_getdetails['identity_number'].".png"; ?>" width="300px" height="300px"/>                                   </td></tr>
                                    <thead> 
                                                 	 	<tr>	  <td  colspan="11">
                                                <strong>NOTES:</strong>
                                                        </td> 				
...
?>
	...
	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-checkiniprs"  role="button" class="orange"data-toggle="modal">	
                                 <button class="btn btn-xs btn-warning">
											  <i class="ace-icon smaller-80 green"></i>fetch from IPRS/IDCHEK.io
</button></a>  
...
<div id="modal-checkiniprs" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
												<div class="table-header">
													<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
														<span class="white">&times;</span>													</button>
									 Check in Database</div>
                                            ...
                                            <div class="modal-body padding">	  <form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="checkiniprs" >
                                          <input type="hidden" id="request_id" name="request_id" value="<?php echo $colname_getrequestid; ?>"/>
                                                           <input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
											<input type="hidden" id="search_id" name="search_id" value="<?php echo $search_ref; ?>"/>
						...
						<br/>
							  ...
							  <br/>	<div class="space-10"></div>
														...
														<label class="col-sm-4">Country</label>
															  ...
															  <div class="col-sm-7"><span id="spryselect1">
															<select class="chosen-select form-control" name="identity_country" id="identity_country" data-placeholder="Choose Country..." required>
...
<button class="btn" type="reset">
									          ...
									          <input type="hidden" name="MM_insert" value="checkiniprs">
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
						...
						<br/>
																...
																<div class="col-sm-8"><span id="sprytextfield1">
																  <input type="text" id="identity_name" name="identity_name"   required/>
											    <span class="textfieldRequiredMsg">*</span></span></div>
...
<?php
 ...
 ?>
    ...
    <input type="hidden" id="status" name="status" value="000"/>
...
<![endif]-->
	...
	$('#editControls2 a').click(function(e) {
		...
		var textval = $("#editor").html();
		$("#editorCopy2").val(textval);
	});

	$("#editor").keyup(function() {
		var value = $(this).html();
		$("#editorCopy2").val(value);
	}).keyup();
	
	$('#checkIt2').click(function(e) {
		e.preventDefault();
		alert($("#editorCopy2").val());
	});
});
