...
if (is_uploaded_file($_FILES['building_photo']['tmp_name'])) {
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = $_POST['building_name']."-".$_POST['search_id']."-".$date_insert; 
$rawname = $_FILES['building_photo']['name'];
 	  "Upload: ".$a."_". $_FILES["building_photo"]["name"];
	  $file="residencephotos/".$a."_". $_FILES["building_photo"]["name"];

    require_once "../../uploads.php";
    $prefix = "building_photo";
    $filenameuploaded = uploadFile($prefix,"individual-residencephotos",$a."_".$_FILES[$prefix]["name"]);
}
else
{
$filenameuploaded = $_POST['building_photo2']; 
}
  $updateSQL = sprintf("UPDATE pel_data_residence SET physical_address=%s, house_number=%s, status=%s, date_added=%s, added_by=%s, street=%s, data_source=%s, data_notes=%s, building_name=%s, building_photo=%s, country=%s WHERE residence_id=%s",
                       GetSQLValueString(strtoupper($_POST['physical_address']), "text"),
                       GetSQLValueString(strtoupper($_POST['house_number']), "text"),
					   ...
					   GetSQLValueString($_POST['building_name'], "text"),
					   GetSQLValueString($filenameuploaded, "text"),
					   GetSQLValueString($_POST['country'], "text"),
					   GetSQLValueString($_POST['residence_id'], "int"));
...
else
{
  ...
  $updateGoTo = "residencycheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
$insertSQL = sprintf("INSERT INTO pel_data_residence (physical_address, house_number, status, date_added, added_by, street, data_source, data_notes, building_name, building_photo, country, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString(strtoupper($_POST['physical_address']), "text"),
                       GetSQLValueString(strtoupper($_POST['house_number']), "text"),
                       GetSQLValueString($_POST['status'], "text"),
                       GetSQLValueString($_POST['added_by'], "text"),
                       ...
                       GetSQLValueString($_POST['data_source'], "text"),
                       GetSQLValueString($_POST['data_notes'], "text"),
					   GetSQLValueString(strtoupper($_POST['building_name']), "text"),
					   GetSQLValueString($filenameuploaded, "text"),
					   GetSQLValueString($_POST['country'], "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_data_residence SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if ((isset($_GET['residence_id'])) && ($_GET['residence_id'] != "")) {
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_data_residence SET status=%s, added_by=%s, date_added=now() WHERE residence_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
					   GetSQLValueString($_GET['fullnames'], "text"),
					    GetSQLValueString($_GET['residence_id'], "int"));
					   mysqli_select_db($connect,$database_connect);
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_data_residence SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Individual Residency Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Individual Residency Details</li>
						</ul><!-- /.breadcrumb -->
...
<div class="row">
									<div class="col-xs-12">
									  ...
									  <h3 align="left" class="header smaller lighter blue">Add Individual Residency Details</h3>
                                      </div>
                                         <!--   <div  class="col-xs-6">
...
$query_getdetails = "SELECT * FROM pel_data_residence WHERE search_id = '".$search_ref."'";
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>RESIDENCY CHECK DETAILS: </strong> 
</h3>
										...
										<thead>
													<tr>									
													  <th>BUILDING NAME:</th>
                                                   	...
                                                   	<tr>
                                                    	<td>
                                                   <a href="#"><?php echo $row_getdetails['building_name']; ?> </a>																								</td>
                                                   <td><?php echo $row_getdetails['physical_address']; ?></td> 
                                                   <td><?php echo $row_getdetails['street']; ?></td>
                                                   <td><?php echo $row_getdetails['house_number']; ?></td>  
	...
	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['residence_id']; ?>" role="button" class="green" data-toggle="modal">	
                                 <button class="btn btn-xs btn-info">  
															<i class="ace-icon fa fa-pencil bigger-120"></i>															</button></a>

   <div id="modal-editdetails-<?php echo $row_getdetails['residence_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
 ...
 <input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
										<input type="hidden" id="residence_id" name="residence_id" value="<?php echo $row_getdetails['residence_id']; ?>"/>
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
																...
																<label class="col-sm-4">Building Physical Address</label>

																<div class="col-sm-8"><span id="sprytextfield1">
																  <input type="text" id="physical_address" name="physical_address" value="<?php echo $row_getdetails['physical_address']; ?>"  required/>
						...
						<br/>
														...
														<label class="col-sm-4">House Number</label>
															  ...
															  <div class="col-sm-8"><span id="sprytextfield4">
															    <input  type="text" value="<?php echo $row_getdetails['house_number']; ?>" id="house_number" name="house_number"  required/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
                               	...
                               	<br/>
														...
														<label class="col-sm-4">Building Name</label>
															  ...
															  <div class="col-sm-7"><span id="sprytextfield5">
															    <input  type="text" value="<?php echo $row_getdetails['building_name']; ?>" id="building_name" name="building_name"  required/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
...
$totalRows_getcountries2 = mysqli_num_rows($getcountries2);
														...
														<label class="col-sm-4">Street</label>
															  ...
															  <div class="col-sm-7"><span id="sprytextfield8">
															    <input  value="<?php echo $row_getdetails['street']; ?>" type="text" id="street" name="street"/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
					<br/>
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Building Photo</label>

														<div class="col-sm-8">
															 Currennt Photo <br/>
                                                             	<img width="200px" height="200px" src="<?php echo $row_getdetails['building_photo']; ?>" alt="DL Photos">
                                                                 
                                                  <input id="building_photo2" name="building_photo2" type="hidden" value="<?php echo $row_getdetails['building_photo']; ?>"/>    
											  </div>
                                               	<br/>
															...
															<div class="space-10"></div> 
                                                               <label class="col-sm-4">Click to Change Image</label>
                                                                <div class="col-sm-8">
															          <input class="form-control"  id="building_photo" name="building_photo" type="file"/>
																</div>
...
<button class="btn" type="reset">
                                ...
                                <?php
                                	if($row_getdetails['status']=='11' || $row_getdetails['status']=='22')
														{
														?>
                                                        <a href="residencycheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&residence_id=<?php echo $row_getdetails['residence_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
                                	...
                                	if($row_getdetails['status']=='00')
														...
														?>
                                                    <a href="residencycheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&residence_id=<?php echo $row_getdetails['residence_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
...
<?php
                                               ...
                                               <thead> 
                                                ...
                                                <strong>NOTES:</strong>
                                                    ...
                                                    <tr>
                                                    <td colspan="4"><img width="400px" src="<?php echo $row_getdetails['building_photo']; ?>">
                                   </td>
                                    ...
                                    <br/>  
                                    	<div class="space-10"></div> 
                                                               <label class="col-sm-4">Upload Photo</label>
                                                                <div class="col-sm-8">
															          <input class="form-control" id="building_photo" name="building_photo" type="file" required/>
																</div>
