...
$colname_getproff_member_id = "-1";
if (isset($_GET['proff_member_id_move'])) {
  $colname_getproff_member_id = $_GET['proff_member_id_move'];
}
...
$rawname = $_FILES['membership_certificate']['name'];
	  ...
	  $file="membershipcertificate/".$a."_". $_FILES["membership_certificate"]["name"];
	   ...
	   move_uploaded_file($_FILES["membership_certificate"]["tmp_name"],
...
$filenameuploaded = $_POST['membership_certificate2']; 
  ...
  $updateSQL = sprintf("UPDATE pel_data_proff_membership SET membership_body=%s, status=%s, membership_number=%s, added_by=%s, search_id=%s, registration_date=%s, data_source=%s, data_notes=%s, membership_status=%s, date_added=now(), membership_certificate=%s WHERE membership_id=%s",
...
$insertSQL = sprintf("INSERT INTO pel_data_proff_membership (membership_body, status, membership_number, added_by, search_id, registration_date,  data_source, data_notes, membership_status,  date_added, membership_certificate) VALUES (%s, %s, %s,%s, %s, %s, %s, %s,  %s, now(), %s)",
                       GetSQLValueString(strtoupper($_POST['membership_body']), "text"),
					   GetSQLValueString("22", "text"),
                       GetSQLValueString(strtoupper($_POST['membership_number']), "text"),
                       GetSQLValueString($_POST['added_by'], "text"),
...
$deleteSQLreject = sprintf("UPDATE pel_data_proff_membership SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s and membership_id=%s",
...
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_data_proff_membership SET status=%s, added_by=%s, date_added=now() WHERE membership_id=%s",
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_data_proff_membership SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s and membership_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
<a href="proffmemberverificationcheck.php?request_id=<?php echo $row_getstudent['request_id']; ?>&moduleid=<?php echo $colname_getmoduleid; ?>" role="button" class="green">	
...
<?php
if (isset($_GET['request_id']) && isset($_GET['moduleid']) && isset($_GET['proff_member_id_move'])) {

$colname_getproff_member_id = $colname_getproff_member_id;
$query_getdetails = "SELECT * FROM pel_data_proff_membership WHERE membership_id = '".$colname_getproff_member_id."'";
...
?>
	   <form enctype="multipart/form-data"  method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails" >
                                      	...
                                      	<input type="hidden" id="search_id" name="search_id" value="<?php echo $row_getstudent['request_ref_number'];?>"/>
