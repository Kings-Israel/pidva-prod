...
$query_getstudent = "SELECT * FROM pel_individual_dl_data WHERE (identity_number = '".$identity_number."' OR license_number = '".$license_number."') AND expiry_date > '".$datetoday."' AND status = '11'";
...
$insertSQL = sprintf("INSERT INTO pel_individual_dl_data (first_name, identity_number, status, date_added, added_by, expiry_date, data_source, data_notes, class, license_number, dl_photo, dl_status, country, search_id) 
VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString(strtoupper($row_getstudent['class']), "text"),
					   GetSQLValueString(strtoupper($row_getstudent['license_number']), "text"),
					   GetSQLValueString($row_getstudent['dl_photo'], "text"),
					   GetSQLValueString($row_getstudent['dl_status'], "text"),
...
{ 
  $updateGoTo = "drivinglicensecheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
else
{
	$errorcode = '<div class="alert alert-danger">
										 ...
										 NO Previous Details of DL found
											<br />
										</div>';
...
if (is_uploaded_file($_FILES['dl_photo']['tmp_name'])) {
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = $_POST['license_number']."-".$_POST['search_id']."-".$date_insert; 
$rawname = $_FILES['dl_photo']['name'];
 	  "Upload: ".$a."_". $_FILES["dl_photo"]["name"];
	  $file="dlphotos/".$a."_". $_FILES["dl_photo"]["name"];
	
	   move_uploaded_file($_FILES["dl_photo"]["tmp_name"],

      "dlphotos/".$a."_". $_FILES["dl_photo"]["name"]);

$filenameuploaded = $a."_".$_FILES["dl_photo"]["name"];


}
else
{
$filenameuploaded = $_POST['dl_photo2']; 
}
  $updateSQL = sprintf("UPDATE pel_individual_dl_data SET first_name=%s, identity_number=%s, status=%s, date_added=%s, added_by=%s, expiry_date=%s, data_source=%s, data_notes=%s, class=%s, license_number=%s, dl_photo=%s, dl_status=%s, country=%s WHERE dl_id=%s",
					   ...
					   GetSQLValueString($_POST['dl_status'], "text"),
...
$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the DL were not updated succesfully.
											<br />
										</div>';
...
VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString($_POST['data_source'], "text"),
                       GetSQLValueString($_POST['data_notes'], "text"),
					   GetSQLValueString(strtoupper($_POST['class']), "text"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_individual_dl_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_individual_dl_data SET status=%s, added_by=%s, date_added=now() WHERE dl_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
					   GetSQLValueString($_GET['fullnames'], "text"),
					    GetSQLValueString($_GET['dl_id'], "int"));
					   mysqli_select_db($connect,$database_connect);
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_individual_dl_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Individual DL Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Individual Driving License Details</li>
						</ul><!-- /.breadcrumb -->
...
$query_getdetails = "SELECT * FROM pel_individual_dl_data WHERE search_id = '".$search_ref."'";
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>INDIVIDUAL DRIVING CHECK DETAILS: </strong> 
</h3>
										...
										<thead>
                                                   ...
                                                   <td><?php echo $row_getdetails['class']; ?></td>
                                                   ...
                                                   <td><?php 
														...
														if($row_getdetails['dl_status']=='VALID')
														{
														?>
														...
														if($row_getdetails['dl_status']=='EXPIRED')
														{
														?>
                                                        <span class="label label-sm label-danger">EXPIRED</span>	
                                                         <?php
														}if($row_getdetails['dl_status']=='NOT FOUND')
														{
														?>
                                                        <span class="label label-sm label-danger">NOT FOUND</span>	
                                                         <?php
														}
	...
	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['dl_id']; ?>" role="button" class="green" data-toggle="modal">	
                                 <button class="btn btn-xs btn-info">  
															<i class="ace-icon fa fa-pencil bigger-120"></i>															</button></a>

   <div id="modal-editdetails-<?php echo $row_getdetails['dl_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
 ...
 <input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
										<input type="hidden" id="dl_id" name="dl_id" value="<?php echo $row_getdetails['dl_id']; ?>"/>
                               	...
                               	<br/>
														...
														<label class="col-sm-4">Class</label>
															  ...
															  <div class="col-sm-7"><span id="sprytextfield5">
															    <input  type="text" value="<?php echo $row_getdetails['class']; ?>" id="class" name="class"  required/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
...
$totalRows_getcountries2 = mysqli_num_rows($getcountries2);
								...
								<br/>
															<div class="space-10"></div>
                                                            	...
                                                            	<label class="col-sm-4">DL Status</label>

															  <div class="col-sm-7"><span id="spryselect3">
															  <select class="chosen-select form-control"  name="dl_status" id="dl_status"  data-placeholder="Choose DL Status..." required>
														    <!--  <select name="client_industry" id="client_industry">-->
                                                           <option  value="<?php echo $row_getdetails['dl_status']; ?>"><?php echo $row_getdetails['dl_status']; ?></option>
														        <option value="">DL Status</option>
                                                                <option value="VALID">VALID</option>
                                                                <option value="EXPIRED">EXPIRED</option>

 <option value="NOT DATA FOUND">NO DATA FOUND</option>
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">DL Photo</label>

														<div class="col-sm-8">
															 Currennt Photo <br/>
                                                             	<img width="200px" height="200px" src="https://pidva.africa/html/individual/dlphotos/<?php echo $row_getdetails['dl_photo']; ?>" alt="DL Photos">
                                                                 
                                                  <input id="dl_photo2" name="dl_photo2" type="hidden" value="<?php echo $row_getdetails['dl_photo']; ?>"/>    
											  </div>
                                               	<br/>
															...
															<div class="space-10"></div> 
                                                               <label class="col-sm-4">Click to Change Image</label>
                                                                <div class="col-sm-8">
															          <input class="form-control"  id="dl_photo" name="dl_photo" type="file"/>
																</div>
...
<button class="btn" type="reset">
                                ...
                                <?php
                                	if($row_getdetails['status']=='11' || $row_getdetails['status']=='22')
														{
														?>
                                                        <a href="drivinglicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&dl_id=<?php echo $row_getdetails['dl_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
                                	...
                                	if($row_getdetails['status']=='00')
														...
														?>
                                                    <a href="drivinglicensecheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&dl_id=<?php echo $row_getdetails['dl_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
...
<?php
                                               ...
                                               <thead> 
                                                ...
                                                <strong>NOTES:</strong>
                                                    ...
                                                    <tr>
                                                    <td colspan="5"><img width="400px" src="http://46.101.16.235/pidva/html/individual/dlphotos/<?php echo $row_getdetails['dl_photo']; ?>">
                                   </td>
...
<div id="modal-checkindb" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
																...
																<div class="col-sm-8"><input type="text" id="license_number" name="license_number"/>
										</div>
