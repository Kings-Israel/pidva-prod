...
$insertSQL = sprintf("INSERT INTO pel_individual_tax_data (first_name, identity_number, status, date_added, added_by, expiry_date, data_source, data_notes, tax_organisation, tax_number, tax_photo, tax_status, country, search_id) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$filenameuploaded = $_POST['tax_photo2']; 
  ...
  $updateSQL = sprintf("UPDATE pel_individual_tax_data SET first_name=%s, identity_number=%s, status=%s, date_added=%s, added_by=%s, expiry_date=%s, data_source=%s, data_notes=%s, tax_organisation=%s, tax_number=%s, tax_photo=%s, tax_status=%s, country=%s WHERE tax_compliance_id=%s",
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_individual_tax_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_individual_tax_data SET status=%s, added_by=%s, date_added=now() WHERE tax_compliance_id=%s",
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_individual_tax_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>INDIVIDUAL TAX COMPLIANCE CHECK DETAILS: </strong> 
</h3>
