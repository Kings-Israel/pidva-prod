...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editdetails")) {
	...
	if (is_uploaded_file($_FILES['membership_certificate']['tmp_name'])) {
		date_default_timezone_set('Africa/Nairobi');
		$date_insert = date('dmYhis');
		$a = $_POST['membership_number'] . "-" . $_POST['search_id'] . "-" . $date_insert;
		$rawname = $_FILES['membership_certificate']['name'];
		"Upload: " . $a . "_" . $_FILES["membership_certificate"]["name"];
		$file = "membershipcertificate/" . $a . "_" . $_FILES["membership_certificate"]["name"];

		move_uploaded_file(
			$_FILES["membership_certificate"]["tmp_name"],

			"membershipcertificate/" . $a . "_" . $_FILES["membership_certificate"]["name"]
		);

		$filenameuploaded = $a . "_" . $_FILES["membership_certificate"]["name"];
	} else {
		$filenameuploaded = $_POST['membership_certificate2'];
	}

	$updateSQL = sprintf(
		"UPDATE pel_data_proff_membership SET membership_body=%s, status=%s, membership_number=%s, added_by=%s, search_id=%s, registration_date=%s, data_source=%s, data_notes=%s, membership_status=%s, date_added=now(), membership_certificate=%s WHERE membership_id=%s",
		GetSQLValueString(strtoupper($_POST['membership_body']), "text"),
		GetSQLValueString("22", "text"),
		GetSQLValueString(strtoupper($_POST['membership_number']), "text"),
		...
		GetSQLValueString($_POST['membership_status'], "text"),
		GetSQLValueString($filenameuploaded, "text"),
		GetSQLValueString($_POST['membership_id'], "text")
	);
	...
	if (mysqli_error($connect)) {
		...
		$updateGoTo = "proffmemberverificationcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
		/* if (isset($_SERVER['QUERY_STRING'])) {
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "newdetails")) {
	...
	$insertSQL = sprintf(
		"INSERT INTO pel_data_proff_membership (membership_body, status, membership_number, added_by, search_id, registration_date,  data_source, data_notes, membership_status,  date_added, membership_certificate) VALUES (%s, %s, %s,%s, %s, %s, %s, %s,  %s, now(), %s)",
		GetSQLValueString(strtoupper($_POST['membership_body']), "text"),
		GetSQLValueString("22", "text"),
		GetSQLValueString(strtoupper($_POST['membership_number']), "text"),
		GetSQLValueString($_POST['added_by'], "text"),
	...
	if (mysqli_error($connect)) {
		$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the Membership havent been added.
											<br />
										</div>';
	} else {
		$updateGoTo = "proffmemberverificationcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
	...
	$deleteSQLreject = sprintf(
		"UPDATE pel_data_proff_membership SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s and membership_id=%s",
		...
		GetSQLValueString($_POST['search_id'], "text"),
		GetSQLValueString($_POST['membership_id'], "int")
	);
	mysqli_select_db($connect, $database_connect);
...
if ((isset($_GET['membership_id'])) && ($_GET['membership_id'] != "")) {
	if ($_GET['status'] == '00') {
		$deleteSQL = sprintf(
			"UPDATE pel_data_proff_membership SET status=%s, added_by=%s, date_added=now() WHERE membership_id=%s",
			GetSQLValueString($_GET['status'], "text"),
			GetSQLValueString($_GET['fullnames'], "text"),
			GetSQLValueString($_GET['membership_id'], "int")
		);
		mysqli_select_db($connect, $database_connect);
...
if ((isset($_GET['search_id_approve'])) && ($_GET['search_id_approve'] != "")) {
	if ($_GET['status'] == '11') {
		...
		$deleteSQL2 = sprintf(
			"UPDATE pel_data_proff_membership SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s and membership_id=%s",
			GetSQLValueString($_GET['status'], "text"),
...
<head>
	...
	<meta charset="utf-8" />
	<title>Individual Profffessional Membership Details Data Management - Peleza Admin</title>
			...
			<div class="main-content-inner">
				...
				<div class="breadcrumbs" id="breadcrumbs">
					...
					<ul class="breadcrumb">
						...
						<li class="active">Individual Membership Details</li>
					</ul><!-- /.breadcrumb -->
...
-->
							<div class="row">
								<div class="col-xs-12">
									...
									<h3 align="left" class="header smaller lighter blue">Add Individual Membership Details</h3>
								</div>
								<!--   <div  class="col-xs-6">
								...
								<div>
									...
									$query_getdetails = "SELECT * FROM pel_data_proff_membership WHERE search_id = '" . $search_ref . "'";
									...
									?>
										...
										<div class="col-lg-12" align="center">
											<h3 align="left" class=" smaller lighter blue"><strong>PROFFESSIONAL MEMBERSHIP CHECK DETAILS: </strong>
											</h3>
											<?php
											do {
											?>
												...
												<table id="simple-table" class="table table-bordered table-hover">
													...
													<tr>
														<td>
															<a href="#"><?php echo $row_getdetails['membership_body']; ?> </a>
														</td>
														<td><?php echo $row_getdetails['registration_date']; ?></td>
														<td><?php echo $row_getdetails['membership_number']; ?></td>
														<td><?php echo $row_getdetails['membership_status']; ?></td>
														...
														<td class="hidden-480">
															<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="proffmemberverificationcheckadd.php?request_id=<?php echo $row_getstudent['request_id']; ?>&proff_member_id_move=<?php echo $row_getdetails['membership_id']; ?>&moduleid=<?php echo $colname_getmoduleid; ?>" role="button" class="green" data-toggle="modal">

																<!--	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['membership_id']; ?>" role="button" class="green" data-toggle="modal">	-->
																<button class="btn btn-xs btn-info">
																	<i class="ace-icon fa fa-pencil bigger-120"></i> </button>
															</a>

															<div id="modal-editdetails-<?php echo $row_getdetails['membership_id']; ?>" class="modal fade" tabindex="-1">
																	...
																	<div class="modal-content">
																		...
																		<div class="modal-body padding">
																			<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="editdetails">
																				...
																				<input type="hidden" id="membership_id" name="membership_id" value="<?php echo $row_getdetails['membership_id']; ?>" />
																				...
																				<label class="col-sm-4">Membership Body</label>

																				<div class="col-sm-8"><span id="sprytextfield1">
																						<input type="text" id="membership_body" name="membership_body" value="<?php echo $row_getdetails['membership_body']; ?>" required />
																				...
																				<br />
																				<div class="space-10"></div>
																				<label class="col-sm-4">Membership Number</label>

																				<div class="col-sm-8"><span id="sprytextfield4">
																						<input type="text" value="<?php echo $row_getdetails['membership_number']; ?>" id="membership_number" name="membership_number" required />
																						<span class="textfieldRequiredMsg">*</span></span></div>
																				...
																				<label class="col-sm-4">Membership Status</label>

																				<div class="col-sm-8">
																					<select class="chosen-select form-control" name="membership_status" id="membership_status" data-placeholder="Choose Membership Status..." required>
																						<!--  <select name="client_industry" id="client_industry">-->
																						<option value="<?php echo $row_getdetails['membership_status']; ?>"><?php echo $row_getdetails['membership_status']; ?></option>
																				...
																				<label class="col-sm-4">Membership Certificate</label>

																				<div class="col-sm-8">
																					Current membership_certificate <br />
																					<img width="200px" height="200px" src="membershipcertificate/<?php echo $row_getdetails['membership_certificate']; ?>" alt="ID membership_certificates">

																					<input id="membership_certificate2" name="membership_certificate2" type="hidden" value="<?php echo $row_getdetails['membership_certificate']; ?>" />
																				</div>
																				<br />
																				...
																				<div class="col-sm-8">
																					<input class="form-control" id="membership_certificate" name="membership_certificate" type="file" />
																				</div>
																				<br />
															...
															?>
																<a href="proffmemberverificationcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=00&membership_id=<?php echo $row_getdetails['membership_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
																		<i class="ace-icon fa fa-trash-o bigger-120"></i> </button></a> <?php
																																	}
																																	if ($row_getdetails['status'] == '00') {

																																		?>
																<a href="proffmemberverificationcheck.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=22&membership_id=<?php echo $row_getdetails['membership_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
												...
												<table id="simple-table" class="table  table-striped  table-bordered table-hover">
													...
													<tr>
														<td width="50%"><img width="200px" src="membershipcertificate/<?php echo $row_getdetails['membership_certificate']; ?>">
														</td>
														<td>
												...
												<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="proffmemberverificationcheckadd.php?fullnames=<?php echo $_SESSION['MM_full_names'] . "(" . $_SESSION['MM_USR_EMAIL'] . ")"; ?>&status=11&search_id_approve=<?php echo $search_ref; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>&membership_id=<?php echo $row_getdetails['membership_id']; ?>"> <button class="btn btn-xs btn-success">
														<i class="ace-icon fa fa-check bigger-120">Approve</i> </button></a>

												<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-addreject-<?php echo $row_getdetails['membership_id']; ?>" role="button" class="blue" data-toggle="modal">
													<button class="btn btn-xs btn-danger"> Reject With Reason </button></a>
												<div id="modal-addreject-<?php echo $row_getdetails['membership_id']; ?>" class="modal fade" tabindex="-1">
													<div class="modal-dialog">
														<div class="modal-content">
															<div class="modal-header no-padding">
																<div class="table-header">
															...
															<div class="modal-body padding">
																<form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="addreject">
																	<input type="hidden" id="request_id" name="request_id" value="<?php echo $colname_getrequestid; ?>" />
																	<input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>" />
																	<input type="text" id="search_id" name="search_id" value="<?php echo $search_ref; ?>" />
																	<input type="text" id="membership_id" name="membership_id" value="<?php echo $row_getdetails['membership_id']; ?>" />
    ...
    -->
								<a href="proffmemberverificationcheckadd.php?request_id=<?php echo $row_getstudent['request_id']; ?>&moduleid=<?php echo $colname_getmoduleid; ?>" role="button" class="green">
									<button class="btn btn-xs btn-primary">
										<i class="ace-icon smaller-80 green"></i>Add Data
									</button></a>
										...
										<div class="modal-content">
											...
											<div class="modal-body padding">
												<form enctype="multipart/form-data" method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal" name="newdetails">
													...
													<div class="col-sm-8"><span id="sprytextfield1">
															<input type="text" id="membership_body" name="membership_body" required />
															<span class="textfieldRequiredMsg">*</span></span></div>
													<br />
													<div class="space-10"></div>
													<label class="col-sm-4">Registration Date</label>
