...
$rawname = $_FILES['photo']['name'];
    ...
    $prefix = "photo";
    $filenameuploaded = uploadFile($prefix,"individual-watchlistphotos",$a."_".$_FILES[$prefix]["name"]);
...
$filenameuploaded = $_POST['photo2']; 
  ...
  $updateSQL = sprintf("UPDATE pel_individual_watchlist_data SET first_name=%s, status=%s, identity_number=%s, added_by=%s, search_id=%s, date_of_birth=%s, country=%s, citizenship=%s, data_source=%s, data_notes=%s, watchlist_status=%s, father_name=%s, date_added=now(), photo=%s WHERE watchlist_id=%s",
...
$insertSQL = sprintf("INSERT INTO pel_individual_watchlist_data (first_name, status, identity_number, added_by, search_id, date_of_birth, country, citizenship, data_source, data_notes, watchlist_status, father_name, date_added, photo) VALUES (%s, %s, %s,%s, %s, %s, %s, %s,  %s, %s, %s, %s, now(), %s)",
                       GetSQLValueString(strtoupper($_POST['first_name']), "text"),
					   GetSQLValueString("22", "text"),
                       GetSQLValueString(strtoupper($_POST['identity_number']), "text"),
                       GetSQLValueString($_POST['added_by'], "text"),
...
$deleteSQLreject = sprintf("UPDATE pel_individual_watchlist_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s and watchlist_id=%s",
...
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_individual_watchlist_data SET status=%s, added_by=%s, date_added=now() WHERE watchlist_id=%s",
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_individual_watchlist_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s and watchlist_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
if($totalRows_getfiles2>0)	  
{
 ...
 {
  ?>	
                                  <td><b><?php echo $row_getfiles2['psmtfile_type']; ?>:</b></td>
                                  <td><?php echo $row_getfiles2['psmtfile_name']; ?></td>


   <?php
 } while ($row_getfiles2 = mysqli_fetch_assoc($getfiles2)); ?> </tr> </table>	
...
$query_getdetails = "SELECT * FROM pel_individual_watchlist_data WHERE search_id = '".$search_ref."'";
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>GLOBAL WATCHLIST CHECK DETAILS: </strong> 
</h3> 
<?php
do
{
...
<td>
                                                   ...
                                                   <td><?php echo $row_getdetails['father_name']; ?></td>
           ...
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
           						...
           						<div class="space-10"></div>
																...
																<div class="col-sm-8"><span id="sprytextfield1">
																  <input type="text" id="first_name" name="first_name" value="<?php echo $row_getstudent['bg_dataset_name']; ?>"  required/>
            ...
            <div class="space-10"></div>
            	<label class="col-sm-4">Fathers Name</label>
															  ...
															  <div class="col-sm-8"><span id="sprytextfield4">
															    <input  type="text" value="<?php echo $row_getdetails['father_name']; ?>" id="father_name" name="father_name"  required/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
					...
					<br/>
						...
						<br/>
								<div class="space-10"></div>
														...
														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="<?php echo $row_getdetails['photo']; ?>" alt="ID Photos">
...
?>
        ...
        <i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="watchlistindividualcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=11&search_id_approve=<?php echo $search_ref; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>&watchlist_id=<?php echo $row_getdetails['watchlist_id']; ?>"> <button class="btn btn-xs btn-success">
							  ...
							  <div id="modal-addreject-<?php echo $row_getdetails['watchlist_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
												<div class="table-header">
													<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
														<span class="white">&times;</span>													</button>
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
                                     	<div class="space-10"></div>
																...
																<div class="col-sm-8"><span id="sprytextfield1">
																  <input class="form-control" type="text" id="first_name" name="first_name" value="<?php echo $row_getstudent['bg_dataset_name']; ?>"  required/>
											    <span class="textfieldRequiredMsg">*</span></span></div>
			<br/>
            <div class="space-10"></div>
            	<label class="col-sm-4">Date of Birth</label>
