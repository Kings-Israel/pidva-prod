...
$a = "PCC-".$_POST['identity_number']."-".$_POST['search_id']."-".$date_insert; 
...
{
$filenameuploaded = $_POST['finger_print_src2']; 
...
date_default_timezone_set('Africa/Nairobi');
$date_insert = date('dmYhis');
$a = "PEL-THUMB-".$_POST['identity_number']."-".$_POST['search_id']."-".$date_insert; 
...
{
$filenameuploaded_individual = $_POST['finger_print_pel2']; 
}


  $updateSQL = sprintf("UPDATE pel_individual_criminal_data SET first_name=%s, identity_number=%s, status=%s, added_by=%s, search_id=%s, country=%s, data_source=%s, data_notes=%s, finger_print_pel=%s, finger_print_src=%s, date_added=now(), clearance_ref_number=%s, criminal_offence_status=%s  WHERE criminal_id=%s",
					   ...
					   GetSQLValueString($_POST['clearance_ref_number'], "text"),
					   GetSQLValueString($_POST['criminal_offence_status'], "text"),
					   GetSQLValueString($_POST['criminal_id'], "text"));
...
$errorcode = '<div class="alert alert-danger">
											...
											<strong>
											 ...
											 Details of the Criminal Check were not updated succesfully.
											<br />
										</div>';
...
else
{
  ...
  $updateGoTo = "criminalcheck.php?request_id=$colname_getrequestid&moduleid=$colname_getmoduleid";
 /* if (isset($_SERVER['QUERY_STRING'])) {
...
$insertSQL = sprintf("INSERT INTO pel_individual_criminal_data (first_name, identity_number, status, added_by, search_id, country, data_source, data_notes, date_added,  finger_print_pel, finger_print_src,clearance_ref_number, criminal_offence_status) VALUES (%s, %s, %s,%s, %s, %s, %s, %s,  now(), %s, %s, %s, %s)",   
                       GetSQLValueString(strtoupper($_POST['first_name']), "text"),
                       GetSQLValueString(strtoupper($_POST['identity_number']), "text"),
                       GetSQLValueString("22", "text"),
					   GetSQLValueString($_POST['added_by'], "text"),
...
$errorcode = '<div class="alert alert-danger">
										 ...
										 Details of the Crimianal Check Data havent been added.
											<br />
										</div>';
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addreject")) {
   ...
   $deleteSQLreject = sprintf("UPDATE pel_individual_criminal_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now(), review_notes=%s WHERE search_id=%s",
...
if($_GET['status']=='00')
{
  $deleteSQL = sprintf("UPDATE pel_individual_criminal_data SET status=%s, added_by=%s, date_added=now() WHERE criminal_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
					   GetSQLValueString($_GET['fullnames'], "text"),
					    GetSQLValueString($_GET['criminal_id'], "int"));
					   mysqli_select_db($connect,$database_connect);
...
if($_GET['status']=='11')
{
   ...
   $deleteSQL2 = sprintf("UPDATE pel_individual_criminal_data SET status=%s, review_status=%s, verified_by=%s, verified_date=now() WHERE search_id=%s",
                       GetSQLValueString($_GET['status'], "text"),
...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Crimianl Check Details Data Management - Peleza Admin</title>
					...
					<div class="breadcrumbs" id="breadcrumbs">
					  <script type="text/javascript">
						...
						<ul class="breadcrumb">
							...
							<li class="active">Crimianl Check Details</li>
						</ul><!-- /.breadcrumb -->
...
<div class="row">
									<div class="col-xs-12">
									  ...
									  <h3 align="left" class="header smaller lighter blue">Add Crimianl Identity Details</h3>
                                      </div>
                                         <!--   <div  class="col-xs-6">
...
$query_getdetails = "SELECT * FROM pel_individual_criminal_data WHERE search_id = '".$search_ref."'";
...
if($totalRows_getdetails>0)
{
	?> 
										...
										<div class="col-lg-12" align="center"> <h3 align="left" class=" smaller lighter blue"><strong>CRIMINAL CHECK CHECK DETAILS: </strong> 
</h3>
										...
										<thead>
                                                   ...
                                                   <td><?php echo $row_getdetails['clearance_ref_number']; ?></td>  
                                                   <td><?php echo $row_getdetails['criminal_offence_status']; ?></td>
	...
	<i class="ace-icon fa fa-hand-o-right icon-animated-hand-pointer blue"></i><a href="#modal-editdetails-<?php echo $row_getdetails['criminal_id']; ?>" role="button" class="green" data-toggle="modal">	
                                 <button class="btn btn-xs btn-info">  
															<i class="ace-icon fa fa-pencil bigger-120"></i>															</button></a>

   <div id="modal-editdetails-<?php echo $row_getdetails['criminal_id']; ?>" class="modal fade" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header no-padding">
 ...
 <input type="hidden" id="moduleid" name="moduleid" value="<?php echo $colname_getmoduleid; ?>"/>
										<input type="hidden" id="criminal_id" name="criminal_id" value="<?php echo $row_getdetails['criminal_id']; ?>"/>
     ...
     <input type="hidden" id="added_by" name="added_by" value="<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>"/>
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
           						...
           						<div class="space-10"></div>
																...
																<label class="col-sm-4">Criminal Offense Status</label>

																<div class="col-sm-8">
															  <select class="chosen-select form-control"  name="criminal_offence_status" id="criminal_offence_status"  data-placeholder="Choose Type..." required>
														    <!--  <select name="client_industry" id="client_industry">-->
                                                           <option  value="<?php echo $row_getdetails['criminal_offence_status']; ?>"><?php echo $row_getdetails['criminal_offence_status']; ?></option>
														        <option value="">Criminal Offence Status</option>
                                                                <option value="NO REPORTED OFFENSES">NO REPORTED OFFENSES</option>
                                                                <option value="OFFENSE FOUND">OFFENSE FOUND</option>
                                                                </select>
											 </div>
						...
						<br/>
														...
														<label class="col-sm-4">Clearance Ref Number</label>

															  <div class="col-sm-8"><span id="sprytextfield4">
															    <input  type="text" value="<?php echo $row_getdetails['clearance_ref_number']; ?>" id="clearance_ref_number" name="clearance_ref_number"/>
														      <span class="textfieldRequiredMsg">*</span></span></div>
...
$totalRows_getcountries2 = mysqli_num_rows($getcountries2);
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">PCC thumb Print</label>
							...
							<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">Peleza Thumb Print</label>

														<div class="col-sm-8">
															 Current Photo <br/>
                                                             	<img width="200px" height="200px" src="<?php echo $row_getdetails['finger_print_pel']; ?>" alt="ID Photos">
...
<button class="btn" type="reset">
                                ...
                                <?php
                                	if($row_getdetails['status']=='11' || $row_getdetails['status']=='22')
														{
														?>
                                                        <a href="criminalcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=00&criminal_id=<?php echo $row_getdetails['criminal_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-danger">
                                	...
                                	if($row_getdetails['status']=='00')
														...
														?>
                                                    <a href="criminalcheck.php?fullnames=<?php echo $_SESSION['MM_full_names']."(".$_SESSION['MM_USR_EMAIL'].")"; ?>&status=22&criminal_id=<?php echo $row_getdetails['criminal_id']; ?>&request_id=<?php echo $colname_getrequestid; ?>&moduleid=<?php echo $colname_getmoduleid; ?>"> <button class="btn btn-xs btn-success">
...
<?php
                                               ...
                                               <thead> 
                                                 	 	<tr>	  <td  colspan="5">
                                                <strong>PEL THUMB PRINT:</strong>
                                                        </td> 				
                                                    <td  colspan="6">
                                                <strong>PCC THUMB PRINT:</strong>
                                                        </td> 
           ...
           <input type="hidden" id="date_added" name="date_added" value="<?php echo date('Y-m-d H:m:s');?>"/>
						...
						<br/>
														...
														<label class="col-sm-4">Clearance Ref Number</label>
															  ...
															  <div class="col-sm-8"><span id="sprytextfield4">
															    <input  type="text"   id="clearance_ref_number" name="clearance_ref_number" />
														      <span class="textfieldRequiredMsg">*</span></span></div>
...
$totalRows_getcountries2 = mysqli_num_rows($getcountries2);
					...
					<br/>
                    						...
                    						<div class="space-10"></div>
														...
														<label class="col-sm-4">PCC finger Print</label>
